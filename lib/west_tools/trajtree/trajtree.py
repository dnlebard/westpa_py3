...
class TrajTreeSet(_trajtree_base):
    ...
    def trace_trajectories(
        self,
        visit,
        get_visitor_state=None,
        set_visitor_state=None,
        vargs=None,
        vkwargs=None,
    ):

        if (get_visitor_state or set_visitor_state) and not (
            get_visitor_state and set_visitor_state
        ):
            raise ValueError(
                "either both or neither of get_visitor_state and set_visitor_state must be specified"
            )
...
class FakeTrajTreeSet(TrajTreeSet):
    def __init__(self):
        ...
        self.childtable = numpy.array(
            [
                numpy.array([2, 3, 4]),  # segment 1
