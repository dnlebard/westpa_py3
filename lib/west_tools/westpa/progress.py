...
class ProgressIndicator:
    ...
    def draw_fancy(self):
        ...
        stream.write(
            "{t.clear_eol}{t.bold}{}{t.normal}{}\n".format(
                operation_text, self._operation or "", t=term
            )
        )
        ...
        if self._extent:
            ...
            stream.write(
                "{t.clear_eol}{t.bold}{}{t.normal}{}\n".format(
                    remaining_text, completion_est, t=term
                )
            )
        ...
        if self.fancy:
            ...
            for _i in range(nlines):
                self.terminal.stream.write(
                    "{t.clear_eol}{t.move_down}".format(t=self.terminal)
                )
            for _i in range(nlines):
    ...
    def _reporter_loop(self):
        ...
        while not self._endloop:
            ...
            if (
                self._last_operation != self._operation
                or time.time() - self._last_update >= self.interval
            ):
                self.draw()
                self.flush_output()
