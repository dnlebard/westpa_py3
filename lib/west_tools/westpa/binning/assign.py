...
class BinMapper:
    ...
    def __repr__(self):
        return "<{} at 0x{:x} with {:d} bins>".format(
            self.__class__.__name__, id(self), self.nbins or 0
        )
...
class RectilinearBinMapper(BinMapper):
    ...
    def boundaries(self, boundaries):
        ...
        for boundset in boundaries:
            ...
            if (db <= 0).any():
                raise ValueError(
                    "boundary set must be strictly monotonically increasing"
                )
            self._boundaries.append(boundarray)
        self._boundlens = numpy.array(
            [len(boundset) for boundset in self._boundaries], dtype=index_dtype
        )
        self.ndim = len(self._boundaries)
        self.nbins = numpy.multiply.accumulate(
            [1] + [len(bounds) - 1 for bounds in self._boundaries]
        )[-1]
        ...
        for index in numpy.ndindex(binspace_shape):
            bounds = [
                (_boundaries[idim][index[idim]], boundaries[idim][index[idim] + 1])
    ...
    def assign(self, coords, mask=None, output=None):
        ...
        if mask is None:
            ...
            raise TypeError(
                "mask [shape {}] has different length than coords [shape {}]".format(
                    mask.shape, coords.shape
                )
            )
...
class PiecewiseBinMapper(BinMapper):
    ...
    def assign(self, coords, mask=None, output=None):
        ...
        fnvals = numpy.empty(
            (len(coord_subset), len(self.functions)), dtype=index_dtype
        )
...
class RecursiveBinMapper(BinMapper):
    ...
    def start_index(self, new_index):
        ...
        if n_not_recursed == self.nbins:
            self._output_map = numpy.arange(
                self._start_index, self._start_index + self.nbins, dtype=index_dtype
            )
            ...
            # set chosen above
            self._output_map[not_recursed] = numpy.arange(
                self._start_index, self._start_index + n_not_recursed, dtype=index_dtype
            )
        else:
            # No un-replaced bins
    ...
    def add_mapper(self, mapper, replaces_bin_at):
        ...
        log.debug(
            "replacing bin {!r} containing {!r} with {!r}".format(
                ibin, replaces_bin_at, mapper
            )
        )
