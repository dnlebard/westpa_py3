...
def get_macrostate_rates(labeled_rates, labeled_pops, extrapolate=True):
    ...
    if extrapolate:
        ...
        if ss is None:
            warnings.warn(
                "no well-defined steady state; using average populations",
                ConsistencyWarning,
            )
...
def estimate_rates(
    ...
    if all_lags:
        twindow = calculate_labeled_fluxes_alllags(
            nstates,
            weights,
            parent_ids,
            bin_assignments,
        ...
        twindow = calculate_labeled_fluxes(
            nstates,
            weights,
            parent_ids,
            bin_assignments,
    ...
    unlabeled_rates[...] = labeled_flux_to_rate(
        unlabeled_fluxes[None, None, :, :], unlabeled_pops[None, :]
    )[0, 0]
