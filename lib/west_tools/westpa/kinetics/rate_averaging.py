...
from westpa.kinetics._kinetics import (
    flux_assign,
    pop_assign,
    calc_rates,
    StreamingStats1D,
...
def process_iter_chunk(bin_mapper, iter_indices, iter_data=None):
    ...
    for iiter, n_iter in enumerate(iter_indices):
        ...
        try:
            ...
            flux_assign(
                weights, prev_init_assignments, new_init_assignments, flux_matrix
            )
            ...
            del (
                prev_init_pcoords,
                new_init_pcoords,
                prev_init_assignments,
                new_init_assignments,
                weights,
            )
...
class RateAverager:
    ...
    def extract_data(self, iter_indices):
        ...
        for n_iter in iter_indices:
            iter_group_name = "iter_{:0{prec}d}".format(
                int(n_iter), prec=self.data_manager.iter_prec
            )
    ...
    def calculate(self, iter_start=None, iter_stop=None, n_blocks=1, queue_size=1):
        ...
        self.stderr_populations = numpy.nan_to_num(
            numpy.sqrt(population_stats.var) / population_stats.n
        )
