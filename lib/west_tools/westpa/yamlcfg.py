...
def warn_dubious_config_entry(
    entry, value, expected_type=None, category=ConfigValueWarning, stacklevel=1
):
    if expected_type:
        warnings.warn(
            "dubious configuration entry {}: {} (expected type {})".format(
                entry, value, expected_type
            ),
            category,
            stacklevel + 1,
        )
        ...
        warnings.warn(
            "dubious configuration entry {}: {}".format(entry, value),
            category,
            stacklevel + 1,
        )
...
def check_bool(value, action="warn"):
    ...
    if not isinstance(value, bool):
        if action == "warn":
            warnings.warn(
                "dubious boolean value {!r}, will be treated as {!r}".format(
                    value, bool(value)
                ),
            ...
            raise ValueError(
                "dubious boolean value {!r}, would be treated as {!r}".format(
                    value, bool(value)
                )
            )
    else:
...
class ConfigItemTypeError(TypeError):
    def __init__(self, key, expected_type, message=None):
        ...
        if message is None:
            message = "configuration item {!r} must have type {!r}".format(
                key, expected_type
            )
...
class YAMLSystem:
    ...
    def bin_target_counts(self, target_counts):
        ...
        self._bin_target_counts = numpy.array(
            target_counts, dtype=numpy.min_scalar_type(maxcount)
        )
