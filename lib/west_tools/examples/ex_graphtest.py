...
tree = trajtree.construct_tree(
    data_manager.current_iteration - 1, get_leaves, data_manager
)
print(
    "tree has {:d} roots, {:d} leaves, and {:d} segments".format(
        len(tree.roots), len(tree.leaves), len(tree)
    )
)
...
print(
    "there are {:d} branch points leading to approximately independent trajectory bundles".format(
        len(leaves_by_branchpoint)
    )
)
# for ((branch,node),leaves) in leaves_by_branchpoint.iteritems():
for (branchnode, subtrees) in leaves_by_branchpoint.items():
    print(
        "  branch point {}:{} has {} independent subtree(s)".format(
            branchnode.n_iter, branchnode.seg_id, len(subtrees)
        )
    )
...
for (root, leaves) in leftovers_by_root.items():
    print(
        "{} trajectories from root {}:{} pruned due to shared history".format(
            len(leaves), root.n_iter, root.seg_id
        )
    )
