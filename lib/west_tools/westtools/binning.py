...
def mapper_from_expr(expr):
    ...
    try:
        ...
        if "has no len" in str(e):
            raise ValueError(
                "invalid bin boundary specification (a list of lists is required)"
            )
        else:
            raise
    else:
...
def mapper_from_dict(ybins):
    ...
    try:
        ...
        raise KeyError(
            "unknown bin mapper type {!r} in YAML file {!r}".format(
                typename, yamlfilename
            )
        )
...
def write_bin_info(
    mapper, assignments, weights, n_target_states, outfile=sys.stdout, detailed=False
):
    ...
    outfile.write(
        "Dynamic range (by bin):         {:g} kT\n".format(
            -math.log(min_bin_weight / max_bin_weight)
        )
    )
    ...
    outfile.write(
        "Dynamic range (by segment):     {:g} kT\n".format(
            -math.log(min_seg_weight / max_seg_weight)
        )
    )
    outfile.write(
        "Norm = {:g}, error in norm = {:g} ({:.0f} epsilon)\n".format(
            norm, enorm, enormeps
        )
    )
    ...
    for ibin, label, nwalkers, total_weight, seg_weights in zip(
        count(), mapper.labels, bincounts, binweights, weights_by_bin
    ):
        if nwalkers > 0:
...
def write_bin_labels(
    ...
    if header:
        dest.write(
            header.format(
                mapper=mapper, classname=mapper.__class__.__name__, nbins=mapper.bins
            )
        )
...
class BinMappingComponent(WESTToolComponent):
    ...
    def process_args(self, args):
        ...
        if not any(
            [bins_from_system, bins_from_expr, bins_from_function, bins_from_h5file]
        ):
        ...
        if bins_from_h5file:
            ...
            self.mapper, self.mapper_pickle, self.mapper_hash = mapper_from_hdf5(
                self.mapper_source_group, self.mapper_source_hash
            )
        ...
        log.info(
            "loaded mapper {!r} from {}".format(self.mapper, self.mapper_source_desc)
        )
        ...
        if self._parse_target_count_args and self.target_counts_required:
            ...
            if args.target_counts is not None:
                self.bin_target_counts = numpy.array(
                    list(map(int, re.split(r"\s*,\s*", args.target_counts)))
                )
                ...
                self.bin_target_counts = numpy.array(
                    list(
                        map(
                            int,
                            re.split(
                                r"\s*,\s*", open(args.target_counts_from, "rt").read()
                            ),
                        )
                    )
                )
                ...
                if self.bin_target_counts is None and self.target_counts_required:
                    raise EnvironmentError(
                        "target counts are required but none have been provided"
                    )
