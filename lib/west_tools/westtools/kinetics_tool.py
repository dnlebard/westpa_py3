    ...
    def process_args(self, args):
        ...
        if self.iter_range.iter_step is None:
            ...
            self.iter_range.iter_step = max(
                1, (self.iter_range.iter_stop - self.iter_range.iter_start) // 10
            )
...
class AverageCommands(WESTKineticsBase):
    ...
    def process_args(self, args):
        ...
        self.mcbs_nsets = (
            args.nsets if args.nsets else mclib.get_bssize(self.mcbs_alpha)
        )
    ...
    def run_calculation(
        ...
        **extra
        ...
        if dim == 2:
            evolution_dataset = numpy.zeros(
                (len(start_pts), nstates, nstates), dtype=ci_dtype
            )
        elif dim == 1:
        ...
        bootstrap_length = 0.5 * (len(start_pts) * (len(start_pts) + 1)) - numpy.delete(
            all_items, numpy.arange(1, len(start_pts) + 1, step_iter)
        )
        if True:
            ...
            for iblock, start in enumerate(start_pts):
                ...
                futures.append(
                    generate_future(self.work_manager, name, eval_block, future_kwargs)
                )
