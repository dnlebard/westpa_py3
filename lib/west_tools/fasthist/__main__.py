...
def _test_double(npts=1024 * 1024, loops=3):
    ...
    for n in range(loops):
        ...
        print(
            "sum of the absolute errors: {} ({} relative, {} per entry)".format(
                errsum, rel_err, errsum_per_item
            )
        )
...
def _test_uint(npts=1024 * 1024, ndim=3, loops=3):
    ...
    print(weights)
    for n in range(loops):
        testdat = numpy.require(
            numpy.random.randint(0, 4, size=(npts, ndim)), numpy.uint16
        )
        print("test data")
        print(testdat)
        mstart = time()
