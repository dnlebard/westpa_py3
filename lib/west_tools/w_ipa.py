...
class WIPI(WESTParallelTool):
    ...
    def process_args(self, args):
        ...
        for ischeme, scheme in enumerate(self.__settings["analysis_schemes"]):
            if (
                ...
                or self.__settings["analysis_schemes"][scheme]["enabled"] == None
            ):
                self.scheme = scheme
        self.data_args = args
            ...
            if self.__settings["analysis_schemes"][scheme]["enabled"]:
                ...
                try:
                    if (
                        ...
                        == True
                        or self.__settings["postanalysis"] == True
                    ):
                ...
                for name in analysis_files:
                    ...
                    if True:
                        if name == "assign":
                            ...
                            if self.ignore_hash == False and (
                                arg_hash != new_hash or self.reanalyze == True
                            ):
                        ...
                        if name == "direct" or name == "reweight":
                            ...
                            if self.ignore_hash == False and (
                                arg_hash != new_hash or reanalyze_kinetics == True
                            ):
                                try:
                                    os.remove(os.path.join(path, "{}.h5".format(name)))
                                except:
    ...
    def reweight(self):
        if (
            self.__settings["analysis_schemes"][str(self.scheme)]["postanalysis"]
            == True
        ):
