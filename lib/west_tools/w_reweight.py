...
def _2D_eval_block(
    ...
    for istate in range(nstates):
        for jstate in range(nstates):
            ...
            dataset = {
                "indices": np.array(list(range(start - 1, stop - 1)), dtype=np.uint16)
            }
...
"""
    ...
    def process_args(self, args):
        ...
        self.sampling_frequency = (
            "iteration"
            if self.assignments_file.attrs["subsampled"] == True
...
class RWReweight(AverageCommands):
    ...
    def generate_reweight_data(self):
        ...
        start_iter, stop_iter, step_iter = (
...
class RWRate(RWReweight):
    subcommand = "kinetics"
    help_text = (
        "Generates rate and flux values from a WESTPA simulation via reweighting."
    )
    default_kinetics_file = "reweight.h5"
    default_output_file = "reweight.h5"
    description = """\
    ...
    def w_postanalysis_reweight(self):
        ...
        submit_kwargs["estimator_kwargs"].update(
            dict(
                ...
                flux=self.flux,
                insert=self.insert,
                bin_last_state_map=np.tile(
                    np.arange(self.nstates, dtype=np.int), self.nbins
                ),
        ...
        self.output_file.replace_dataset(
            "avg_conditional_fluxes", data=avg_conditional_fluxes[1]
        )
        ...
        rate_evol = self.run_calculation(
            eval_block=_2D_eval_block, name="Rate Evolution", dim=2, **submit_kwargs
        )
        rate_evol["expected"] *= self.npts - 1
        rate_evol["ci_ubound"] *= self.npts - 1
        rate_evol["ci_lbound"] *= self.npts - 1
        self.output_file.replace_dataset(
            "rate_evolution", data=rate_evol, shuffle=True, compression=9
        )

        submit_kwargs["estimator_kwargs"]["return_obs"] = "F"
        flux_evol = self.run_calculation(
            eval_block=_2D_eval_block,
            name="Conditional Flux Evolution",
            dim=2,
            **submit_kwargs
        )
        ...
        self.output_file.replace_dataset(
            "conditional_flux_evolution", data=rate_evol, shuffle=True, compression=9
        )
...
"""
    ...
    def w_postanalysis_stateprobs(self):
        ...
        start_pts = list(range(start_iter, stop_iter, step_iter))
        indices = np.array(
            list(range(start_iter - 1, stop_iter - 1, step_iter)), dtype=np.uint16
        )
        submit_kwargs = dict(
            pi=pi,
            nstates=self.nstates,
        ...
        avg_color_probs = self.run_calculation(
            eval_block=_1D_eval_block,
            name="Average Color (Ensemble) Probability",
            dim=1,
            do_averages=True,
            **submit_kwargs
        )
        ...
        avg_state_probs = self.run_calculation(
            eval_block=_1D_eval_block,
            name="Average State Probability",
            dim=1,
            do_averages=True,
            **submit_kwargs
        )
        ...
        color_evol = self.run_calculation(
            eval_block=_1D_eval_block,
            name="Color (Ensemble) Probability Evolution",
            dim=1,
            **submit_kwargs
        )
        self.output_file.replace_dataset(
            "color_prob_evolution", data=color_evol, shuffle=True, compression=9
        )

        submit_kwargs["estimator_kwargs"]["return_obs"] = "S"
        state_evol = self.run_calculation(
            eval_block=_1D_eval_block,
            name="State Probability Evolution",
            dim=1,
            **submit_kwargs
        )
        self.output_file.replace_dataset(
            "state_pop_evolution", data=state_evol, shuffle=True, compression=9
        )
        ...
        if False:
            ...
            hist = self.output_file.replace_dataset(
                "histograms", data=bin_pops, shuffle=True, compression=9
            )
            ...
            binbounds.append(
                ast.literal_eval(self.assignments_file["bin_labels"][-1])[0][1]
            )
            # binbounds.append(binbounds[-1]+1)
            for i in range(1, len(binbounds)):
                midpoints.append(
                    ((binbounds[i] - binbounds[i - 1]) / 2) + binbounds[i - 1]
                )
            self.output_file.replace_dataset(
                "binbounds_0", data=binbounds, shuffle=True, compression=9
            )
            self.output_file.replace_dataset(
                "midpoints_0", data=midpoints, shuffle=True, compression=9
            )
            self.output_file.replace_dataset(
                "n_iter",
                data=list(range(start_iter, stop_iter)),
                shuffle=True,
                compression=9,
            )
