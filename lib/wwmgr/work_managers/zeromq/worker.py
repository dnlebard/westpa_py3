...
class ZMQWorker(ZMQCore):
    ...
    def identify(self, rr_socket):
        if (
            self.master_id is None
            or self.identified
            or self.timers.expired(TIMEOUT_MASTER_BEACON)
        ):
            return
        self.send_message(
            rr_socket, Message.IDENTIFY, payload=self.get_identification()
        )
        self.recv_ack(
            rr_socket, timeout=self.master_beacon_period * self.timeout_factor * 1000
        )
    ...
    def request_task(self, rr_socket, task_socket):
        if self.master_id is None:
            ...
            reply = self.recv_message(
                rr_socket,
                timeout=self.master_beacon_period * self.timeout_factor * 1000,
            )
    ...
    def comm_loop(self):
        ...
        try:
            ...
            while True:
                ...
                # case.
                poll_results = dict(
                    poller.poll((timers.next_expiration_in() or 0.001) * 1000)
                )

                if poll_results and not peer_found:
                    timers.remove_timer("startup_timeout")
                    peer_found = True
                    timers.change_duration(
                        TIMEOUT_MASTER_BEACON,
                        self.master_beacon_period * self.timeout_factor,
                    )
                ...
                if not peer_found and timers.expired("startup_timeout"):
                    self.log.error(
                        "startup phase elapsed with no contact from master; shutting down"
                    )
                    break
        ...
        if self.executor_process.is_alive():
            ...
            self.log.debug(
                "worker process {:d} terminated gracefully with code {:d}".format(
                    pid, self.executor_process.exitcode
                )
            )
    ...
    def startup(self, process_index=None):
        ...
        self.executor_process = multiprocessing.Process(
            target=executor.startup, args=(process_index,)
        )
