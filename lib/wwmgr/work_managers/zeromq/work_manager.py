...
class ZMQWorkManager(ZMQCore, WorkManager, IsNode):
    ...
    def add_wm_args(cls, parser, wmenv=None):
        ...
        wm_group = parser.add_argument_group(
    ...
    def from_environ(cls, wmenv=None):
        ...
        master_heartbeat = wmenv.get_val(
            "zmq_master_heartbeat", cls.default_master_heartbeat, float
        )
        worker_heartbeat = wmenv.get_val(
            "zmq_worker_heartbeat", cls.default_worker_heartbeat, float
        )
        timeout_factor = wmenv.get_val(
            "zmq_timeout_factor", cls.default_timeout_factor, float
        )
        startup_timeout = wmenv.get_val(
            "zmq_startup_timeout", cls.default_startup_timeout, float
        )
        ...
        if mode == "master":
            ...
            upstream_rr_endpoint = wmenv.get_val(
                "zmq_upstream_rr_endpoint", upstream_info.get("rr_endpoint")
            )
            upstream_ann_endpoint = wmenv.get_val(
                "zmq_upstream_ann_endpoint", upstream_info.get("ann_endpoint")
            )

            if not (upstream_rr_endpoint and upstream_ann_endpoint):
                raise ZMQWMEnvironmentError(
                    "at least one upstream endpoint unspecified"
                )
            ...
            upstream_rr_endpoint = cls.canonicalize_endpoint(
                upstream_rr_endpoint, allow_wildcard_host=False
            )
            upstream_ann_endpoint = cls.canonicalize_endpoint(
                upstream_ann_endpoint, allow_wildcard_host=False
            )
            ...
            instance = ZMQNode(
                upstream_ann_endpoint=upstream_ann_endpoint,
        ...
        downstream_ann_endpoint = cls.canonicalize_endpoint(
            wmenv.get_val(
                "zmq_downstream_ann_endpoint", "tcp://*:{}".format(randport())
            )
        )
    ...
    def update_worker_information(self, msg):
        ...
        try:
            ...
            self.worker_timeouts.add_timer(
                msg.src_id, self.worker_beacon_period * self.timeout_factor
            )
    ...
    def check_workers(self):
        ...
        for expired_worker_id in expired_worker_ids:
            ...
            self.log.error(
                "no contact from worker {}".format(
                    expired_worker_id, worker_description
                )
            )
    ...
    def remove_worker(self, worker_id):
        try:
            ...
            self.log.error(
                "aborting task {!r} running on expired worker {!s}".format(
                    expired_task, worker_id
                )
            )
            future = self.futures.pop(expired_task.task_id)
            future._set_exception(
                ZMQWorkerMissing("worker running this task disappeared")
            )
    ...
    def shutdown_clear_tasks(self):
        ...
        while self.futures:
            ...
            future._set_exception(
                ZMQWMEnvironmentError("work manager shut down during task")
            )
        self.futures = None
    ...
    def comm_loop(self):
        ...
        timers.add_timer(
            "worker_timeout_check", self.worker_beacon_period * self.timeout_factor
        )
        ...
        try:
            ...
            while True:
                ...
                if not peer_found and timers.expired("startup_timeout"):
                    self.log.error(
                        "startup phase elapsed with no contact from workers; shutting down"
                    )
                    while self.futures:
