...
class ZMQNode(ZMQCore, IsNode):
    def __init__(
        self, upstream_rr_endpoint, upstream_ann_endpoint, n_local_workers=None
    ):
    ...
    def comm_loop(self):
        ...
        self.log.debug(
            "connecting upstream_rr_endpoint = {!r}".format(self.upstream_rr_endpoint)
        )
        ...
        self.log.debug(
            "connecting upstream_ann_endpoint = {!r}".format(self.upstream_ann_endpoint)
        )
            ...
            while True:
                ...
                if ann_monitor in poll_results:
                    ...
                    if Message.SHUTDOWN in message_tags:
                        self.log.debug("shutdown received")
                        break
                    if not peer_found and (
                        Message.MASTER_BEACON in message_tags
                        or Message.TASKS_AVAILABLE in message_tags
                    ):
                        peer_found = True
                ...
                if not peer_found and timers.expired("startup_timeout"):
                    self.log.error(
                        "startup phase elapsed with no contact from peer; shutting down"
                    )
                    break
