...
class CommonWorkManagerTests:
    ...
    def test_submit_many(self):
        futures = self.work_manager.submit_many(
            [(will_succeed, (), {}) for i in range(self.MED_TEST_SIZE)]
        )
    ...
    def test_as_completed(self):
        ...
        futures = [
            self.work_manager.submit(identity, args=(i,))
            for i in range(self.MED_TEST_SIZE)
        ]
        output = set(
            future.get_result() for future in self.work_manager.as_completed(futures)
        )
        assert input == output
    ...
    def test_submit_as_completed(self):
        ...
        output = set(
            future.get_result()
            for future in self.work_manager.submit_as_completed(task_generator, 10)
        )
        assert input == output
    ...
    def test_wait_all(self):
        ...
        output = set(
            future.get_result() for future in self.work_manager.wait_all(futures)
        )
        assert input == output
