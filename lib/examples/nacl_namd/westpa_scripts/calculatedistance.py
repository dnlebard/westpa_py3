...
dist = MDAnalysis.analysis.distances.self_distance_array(
    parent_nacl.atoms.positions, box=parent_universe.dimensions
)[0]
...
for ts in current_universe.trajectory:
    dist = MDAnalysis.analysis.distances.self_distance_array(
        nacl.atoms.positions, box=current_universe.dimensions
    )[0]
    print("{:.03f}".format(dist))
