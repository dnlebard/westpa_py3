...
def eucl_dist(p, centers):
    d = np.array(
        [np.linalg.norm(np.array(p - c, dtype=float)) for c in centers],
        dtype=np.float32,
    )
...
class System(WESTSystem):
    def initialize(self):
        ...
        self.bin_mapper = wexplore.WExploreBinMapper(
            n_regions=[10, 10, 10], d_cut=[5, 2.0, 0.8], dfunc=eucl_dist
        )
        ...
        self.bin_target_counts = self.bin_mapper.balance_replicas(
            self.max_replicas, np.array([0,], np.int_)
        )
