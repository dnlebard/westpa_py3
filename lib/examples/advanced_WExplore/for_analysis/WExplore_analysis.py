...
print(
    "There are {0} big regions, {1} medium regions, and {2} small regions".format(
        len(big_regions), len(med_regions), len(sml_regions)
    )
)
...
print(
    "The minimum non-zero distance between images is: {0}".format(
        dists[np.nonzero(dists)].min()
    )
)
