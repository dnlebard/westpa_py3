...
def run(platform_name, deviceid, two):
    ...
    print(
        "Time step: ", (wcadimer.stable_timestep * 2.0).in_units_of(units.femtoseconds)
    )
    ...
    integrator = openmm.LangevinIntegrator(
        wcadimer.temperature, wcadimer.collision_rate, 2.0 * wcadimer.stable_timestep
    )
    ...
    if two:
        while not_obs[0] or not_obs[1]:
            ...
            np.save(
                "bstates/init_coords{}.npy".format(tag), coordinates / units.nanometers
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Device id for equilibration if using a GPU",
    )
    parser.add_argument(
        "--two",
        action="store_true",
        default=False,
        help="Create initial states in both basins",
    )
