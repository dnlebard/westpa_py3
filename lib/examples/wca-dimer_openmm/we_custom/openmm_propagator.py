...
class OpenMMPropagator(WESTPropagator):
    ...
    def get_pcoord(self, state):
        ...
        if isinstance(state, BasisState):
            ...
            bstate_data_ref = self.makepath(
                self.basis_state_ref_template, template_args
            )
    ...
    def propagate(self, segments):
        ...
        platform_properties = {
            key: value
            for key, value in self.platform_properties.items()
            if key.startswith(self.platform.getName())
        }
        ...
        block_coordinates = np.empty(
            (len(segments), self.mmsystem.getNumParticles(), 3)
        )
        ...
        for si, segment in enumerate(segments):
            ...
            if segment.initpoint_type == Segment.SEG_INITPOINT_CONTINUES:
                ...
                initial_coords = units.Quantity(
                    parent_coords[segment.parent_id], units.nanometer
                )
                initial_velocs = units.Quantity(
                    parent_velocs[segment.parent_id], units.nanometer / units.picosecond
                )
                ...
                basis_fname = os.path.join(
                    os.environ["WEST_SIM_ROOT"],
                    "bstates",
                    "init_coords{}.npy".format(tag),
                )
            ...
            for istep in range(1, self.nblocks):
                ...
                assert coordinates[istep].max() < 20.0, "max coord: {}".format(
                    coordinates[istep].max()
                )
        ...
        np.savez_compressed(
            "traj_segs/iter_{:06d}_{:06d}.npz".format(
                segments[0].n_iter, block_seg_ids[0]
            ),
