...
def minimize(platform, system, positions):
    ...
    print(
        "initial potential: %12.3f kcal/mol"
        % (initial_potential / units.kilocalories_per_mole)
    )
    # Minimize.
    ...
    # Report
    print(
        "final potential  : %12.3f kcal/mol"
        % (final_potential / units.kilocalories_per_mole)
    )

    return positions


def equilibrate_langevin(
    system, timestep, collision_rate, temperature, sqrt_kT_over_m, coordinates, platform
):
    nsteps = 5000
...
if __name__ == "__main__":
    ...
    print(
        "equilibrate timestep = %.1f fs, switch timestep = %.1f fs"
        % (equilibrate_timestep / units.femtoseconds, timestep / units.femtoseconds)
    )
    ...
    sqrt_kT_over_m = units.Quantity(
        numpy.zeros([nparticles, 3], numpy.float64), units.nanometers / units.picosecond
    )
    for particle_index in range(nparticles):
        sqrt_kT_over_m[particle_index, :] = units.sqrt(
            kT / masses[particle_index, 0]
    ...
    if not os.path.exists(netcdf_filename):
        ...
        # Equilibrate.
        print("Equilibrating...")
        coordinates, velocities = equilibrate_langevin(
            system,
    ...
    while iteration < niterations:
        ...
        if iteration % 100 == 0:
            ...
            print(
                "Dynamics %.1f A -> %.1f A (barrier at %.1f A)"
                % (
                    initial_distance / units.angstroms,
                    final_distance / units.angstroms,
                    (r0 + w) / units.angstroms,
                )
            )
