...
def create_parser():
    ...
    parser.add_argument(
        "-i", "--integrator", required=True, help="OpenMM integrator xml file"
    )
    parser.add_argument(
        "-p",
        "--platform",
        default="CUDA",
        choices=["CUDA", "OpenCL", "Reference", "CPU"],
        help="OpenMM platform name",
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=int,
        help="write frequency for output files",
    )
    parser.add_argument(
        "-o", "--output", required=True, help="base name for output files"
    )
    parser.add_argument(
        "-n", "--nsteps", type=int, required=True, help="Number of steps to run"
    )
