...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--first_iter", required=True, type=int, help="First iteration to remove"
    )
    parser.add_argument(
        "--last_iter", required=True, type=int, help="Final iteration to remove"
    )
    parser.add_argument(
        "-D",
        "--dir",
        default="traj_segs",
        help="Base directory containing trajectory seg directories",
    )
    parser.add_argument(
        "-i", "--iter_prec", default=6, type=int, help="Width of iteration for padding"
    )
