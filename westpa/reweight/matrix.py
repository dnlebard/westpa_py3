...
def calc_stats(
    bin_assignments, weights, fluxes, populations, trans, mask, sampling_frequency
):
    fluxes.fill(0.0)
        ...
        for iiter, n_iter in enumerate(range(start_iter, stop_iter)):
            ...
            bin_assignments = np.require(
                self.assignments_file["assignments"][
                    assignment_iiter + np.s_[:nsegs, :npts]
                ],
                dtype=index_dtype,
            )
            ...
            macrostate_assignments = np.require(
                self.assignments_file["trajlabels"][
                    macrostate_iiter + np.s_[:nsegs, :npts]
                ],
                dtype=index_dtype,
            )
            ...
            flux_iter_grp.create_dataset(
                "flux", data=fluxes_sp.data, dtype=weight_dtype
            )
            ...
            if (
                self.assignments_file.attrs["subsampled"] == True
                or self.sampling_frequency == "iteration"
            ):
