...
class Segment:
    ...
    def __init__(
        ...
        self.endpoint_type = (
            int(endpoint_type) if endpoint_type else self.SEG_ENDPOINT_UNSET
        )
    ...
    def __repr__(self):
        return (
            "<%s(%s) n_iter=%r seg_id=%r weight=%r parent_id=%r wtg_parent_ids=%r pcoord[0]=%r pcoord[-1]=%r>"
            % (
...
Segment.statuses.update(
    {
        _attr: getattr(Segment, _attr)
        for _attr in dir(Segment)
        if _attr.startswith("SEG_STATUS_")
    }
)
Segment.initpoint_types.update(
    {
        _attr: getattr(Segment, _attr)
        for _attr in dir(Segment)
...
Segment.status_names.update(
    {
        getattr(Segment, _attr): _attr
        for _attr in dir(Segment)
        if _attr.startswith("SEG_STATUS_")
    }
)
...
Segment.endpoint_type_names.update(
    {
        getattr(Segment, _attr): _attr
        for _attr in dir(Segment)
        if _attr.startswith("SEG_ENDPOINT_")
