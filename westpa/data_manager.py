...
from westpa.states import BasisState, TargetState, InitialState
...
seg_id_dtype = (
    numpy.int64
)  # Up to 9 quintillion segments per iteration; signed so that initial states can be stored negative
...
ibstate_index_dtype = numpy.dtype(
    [("iter_valid", numpy.uint), ("n_bstates", numpy.uint), ("group_ref", h5ref_dtype)]
)

# Basis state index type
bstate_dtype = numpy.dtype(
    [
...
istate_dtype = numpy.dtype(
    [
        (
            "iter_created",
            numpy.uint,
        ),  # Iteration during which this state was generated (0 for at w_init)
        ("iter_used", numpy.uint),  # When this state was used to start a new trajectory
        (
            "basis_state_id",
            seg_id_dtype,
        ),  # Which basis state this state was generated from
        (
            "istate_type",
            istate_type_dtype,
        ),  # What type this initial state is (generated or basis)
        (
            "istate_status",
            istate_status_dtype,
        ),  # Whether this initial state is ready to go
    ]
)

tstate_index_dtype = numpy.dtype(
    [
        ("iter_valid", numpy.uint),  # Iteration when this state list is valid
        ("n_states", numpy.uint),
...
binning_index_dtype = numpy.dtype(
    [("hash", binhash_dtype), ("pickle_len", numpy.uint32)]
)
...
class WESTDataManager:
    ...
    def process_config(self):
        ...
        self.we_h5filename = config.get_path(
            ["west", "data", "west_data_file"], default=self.default_we_h5filename
        )
        ...
        self.iter_prec = config.get(
            ["west", "data", "iter_prec"], self.default_iter_prec
        )
        self.aux_compression_threshold = config.get(
            ["west", "data", "aux_compression_threshold"],
            self.default_aux_compression_threshold,
        )
        self.flush_period = config.get(
            ["west", "data", "flush_period"], self.default_flush_period
        )

        # Process dataset options
        dsopts_list = config.get(["west", "data", "datasets"]) or []
        for dsopts in dsopts_list:
            dsopts = normalize_dataset_options(
                dsopts, path_prefix="auxdata" if dsopts["name"] != "pcoord" else ""
            )
            try:
    ...
    def iter_group_name(self, n_iter, absolute=True):
        if absolute:
            return "/iterations/iter_{:0{prec}d}".format(
                int(n_iter), prec=self.iter_prec
            )
        else:
    ...
    def require_iter_group(self, n_iter):
        ...
        with self.lock:
            iter_group = self.we_h5file.require_group(
                "/iterations/iter_{:0{prec}d}".format(int(n_iter), prec=self.iter_prec)
            )
    ...
    def get_iter_group(self, n_iter):
        with self.lock:
            try:
                ...
                return self.we_h5file[
                    "/iter_{:0{prec}d}".format(int(n_iter), prec=self.iter_prec)
                ]
    ...
    def open_backing(self, mode=None):
        ...
        if not self.we_h5file:
            log.debug(
                "attempting to open {} with mode {}".format(self.we_h5filename, mode)
            )
            self.we_h5file = h5io.WESTPAH5File(
                self.we_h5filename, mode, driver=self.we_h5file_driver
            )
            ...
            if "west_iter_prec" in h5file_attr_keys:
                ...
                log.info(
                    "iteration precision not stored in HDF5; using {:d}".format(
                        self.iter_prec
                    )
                )
            ...
            log.debug(
                "opened WEST HDF5 file version {:d}".format(self.we_h5file_version)
            )

    def prepare_backing(self):  # istates):
        """Create new HDF5 file"""
        self.we_h5file = h5py.File(
            self.we_h5filename, "w", driver=self.we_h5file_driver
        )
        ...
        with self.flushing_lock():
            ...
            self.we_h5file["/"].create_dataset(
                "summary", shape=(1,), dtype=summary_table_dtype, maxshape=(None,)
            )
    ...
    def save_target_states(self, tstates, n_iter=None):
        ...
        if tstates:
            ...
            state_pcoords = numpy.empty(
                (len(tstates), system.pcoord_ndim), dtype=system.pcoord_dtype
            )
        ...
        with self.lock:
            ...
            try:
                ...
                master_index = master_group.create_dataset(
                    "index", shape=(1,), maxshape=(None,), dtype=tstate_index_dtype
                )
                n_sets = 1
            else:
    ...
    def get_target_states(self, n_iter):
        ...
        with self.lock:
            ...
            if tstate_group is not None:
                ...
                tstates = [
                    TargetState(
                        state_id=i, label=str(row["label"]), pcoord=pcoord.copy()
                    )
                    for (i, (row, pcoord)) in enumerate(
                        zip(tstate_index, tstate_pcoords)
                    )
                ]
            else:
                tstates = []
    ...
    def create_ibstate_group(self, basis_states, n_iter=None):
        ...
        with self.lock:
            ...
            try:
                ...
                master_index = master_group.create_dataset(
                    "index", dtype=ibstate_index_dtype, shape=(1,), maxshape=(None,)
                )
                n_sets = 1
            else:
                n_sets = len(master_index) + 1
            ...
            if basis_states:
                ...
                state_pcoords = numpy.empty(
                    (len(basis_states), system.pcoord_ndim), dtype=system.pcoord_dtype
                )
    ...
    def create_initial_states(self, n_states, n_iter=None):
        ...
        with self.lock:
            ...
            try:
                ...
                istate_index = ibstate_group.create_dataset(
                    "istate_index",
                    dtype=istate_dtype,
                    shape=(n_states,),
                    maxshape=(None,),
    ...
    def update_initial_states(self, initial_states, n_iter=None):
        ...
        with self.lock:
            ...
            pcoord_vals = numpy.empty(
                (len(initial_states), system.pcoord_ndim), dtype=system.pcoord_dtype
            )
            for i, initial_state in enumerate(initial_states):
                index_entries[i]["iter_created"] = initial_state.iter_created
                index_entries[i]["iter_used"] = (
                    initial_state.iter_used or InitialState.ISTATE_UNUSED
                )
                index_entries[i]["basis_state_id"] = (
                    initial_state.basis_state_id
                    if initial_state.basis_state_id is not None
                    else -1
                )
                index_entries[i]["istate_type"] = (
                    initial_state.istate_type or InitialState.ISTATE_TYPE_UNSET
                )
                index_entries[i]["istate_status"] = (
                    initial_state.istate_status or InitialState.ISTATE_STATUS_PENDING
                )
    ...
    def get_initial_states(self, n_iter=None):
        ...
        with self.lock:
            ...
            for state_id, (state, pcoord) in enumerate(
                zip(istate_index, istate_pcoords)
            ):
                states.append(
                    InitialState(
                        state_id=state_id,
    ...
    def get_segment_initial_states(self, segments, n_iter=None):
        ...
        with self.lock:
            ...
            for state_id, state, pcoord in zip(
                sorted_istate_ids, istate_rows, istate_pcoords
            ):
                istate = InitialState(
                    state_id=state_id,
                    basis_state_id=int(state["basis_state_id"]),
        ...
        with self.lock:
            ...
            while istart < n_index_entries and len(states) < n_states:
                ...
                for ci in range(len(istate_chunk)):
                    ...
                    state_id = istart + ci
                    if (
                        row["iter_used"] == ISTATE_UNUSED
                        and row["istate_status"] == ISTATE_STATUS_PREPARED
                    ):
                        istate = InitialState(
                            state_id=state_id,
    ...
    def prepare_iteration(self, n_iter, segments):
        ...
        log.debug(
            "preparing HDF5 group for iteration %d (%d segments)"
            % (n_iter, len(segments))
        )
        ...
        with self.lock:
            ...
            seg_index_table_ds = iter_group.create_dataset(
                "seg_index", shape=(n_particles,), dtype=seg_index_dtype
            )
            ...
            summary_row["norm"] = numpy.add.reduce(
                list(map(attrgetter("weight"), segments))
            )
            summary_table[n_iter - 1] = summary_row

            # pcoord is indexed as [particle, time, dimension]
            pcoord_opts = self.dataset_options.get(
                "pcoord", {"name": "pcoord", "h5path": "pcoord", "compression": False}
            )
            shape = (n_particles, pcoord_len, pcoord_ndim)
            pcoord_ds = create_dataset_from_dsopts(
                iter_group, pcoord_opts, shape, pcoord_dtype
            )
            ...
            if total_parents > 0:
                wtgraph_ds = iter_group.create_dataset(
                    "wtgraph",
                    (total_parents,),
                    seg_id_dtype,
                    compression="gzip",
                ...
                for (seg_id, segment) in enumerate(segments):
                    ...
                    assert set(parents[offset : offset + extent]) == set(
                        segment.wtg_parent_ids
                    )
    ...
    def update_segments(self, n_iter, segments):
        ...
        with self.lock:
            ...
            pcoord_entries = numpy.empty(
                (n_segments, pcoord_len, pcoord_ndim), dtype=pcoord_dtype
            )

            pc_msel = h5s.create_simple(
                pcoord_entries.shape, (h5s.UNLIMITED,) * pcoord_entries.ndim
            )
            pc_msel.select_all()
            ...
            for iseg in range(n_segments):
                ...
                pc_fsel.select_hyperslab(
                    (seg_id, 0, 0), (1, pcoord_len, pcoord_ndim), op=op
                )
            ...
            for (iseg, (segment, ientry)) in enumerate(
                zip(segments, seg_index_entries)
            ):
                ientry["status"] = segment.status
                ientry["endpoint_type"] = (
                    segment.endpoint_type or Segment.SEG_ENDPOINT_UNSET
                )
            ...
            if dsets:
                for (dsname, (shape, dtype)) in dsets.items():
                    ...
                    try:
                        ...
                        dsopts = normalize_dataset_options(
                            {"name": dsname}, path_prefix="auxdata"
                        )
                    ...
                    for segment in segments:
                        try:
                            ...
                            source_sel = h5s.create_simple(
                                auxdataset.shape, (h5s.UNLIMITED,) * source_rank
                            )
                            source_sel.select_all()
                            dest_sel = dset.id.get_space()
                            dest_sel.select_hyperslab(
                                (segment.seg_id,) + (0,) * source_rank,
                                (1,) + auxdataset.shape,
                            )
        ...
        with self.lock:
            ...
            for iseg, (seg_id, row) in enumerate(zip(seg_ids, seg_index_entries)):
                ...
                if file_version < 5:
                    ...
                    wtg_parent_ids = all_parent_ids[
                        wtg_offset : wtg_offset + wtg_n_parents
                    ]
    ...
    def save_new_weight_data(self, n_iter, new_weights):
        ...
        for ientry, nwentry in enumerate(new_weights):
            ...
            row["target_state_id"] = (
                nwentry.target_state_id if nwentry.target_state_id is not None else -1
            )
            row["initial_state_id"] = (
                nwentry.initial_state_id if nwentry.initial_state_id is not None else -1
            )
    ...
    def get_bin_mapper(self, hashval):
        ...
        with self.lock:
            ...
            try:
                ...
                raise KeyError(
                    "hash {} not found. Could not retrieve binning group".format(
                        hashval
                    )
                )
    ...
    def save_bin_mapper(self, hashval, pickle_data):
        ...
        with self.lock:
            ...
            try:
                ...
                index = binning_group.create_dataset(
                    "index", shape=(1,), maxshape=(None,), dtype=binning_index_dtype
                )
...
def create_dataset_from_dsopts(
    group,
    dsopts,
    shape=None,
    dtype=None,
    data=None,
    autocompress_threshold=None,
    n_iter=None,
):
    ...
    if not shape and data is None:
        ...
        raise ValueError(
            "both shape and dtype must be provided when data is not provided"
        )
    elif shape and data is not None and not data.shape == shape:
        raise ValueError(
            "explicit shape {!r} does not match data shape {!r}".format(
                shape, data.shape
            )
        )
    ...
    if chunks_directive is None:
        ...
        chunks = tuple(
            chunks_directive[i] if chunks_directive[i] <= shape[i] else shape[i]
    ...
    if log.isEnabledFor(logging.DEBUG):
        log.debug(
            "requiring aux dataset {!r}, shape={!r}, opts={!r}".format(
                h5_dsname, shape, opts
            )
        )
    ...
    if "file" in list(dsopts.keys()):
        ...
        if not dsopts["h5path"] in h5group:
            h5group[dsopts["h5path"]] = h5py.ExternalLink(
                dsopts["file"].format(n_iter=n_iter),
                ("/" + "iter_" + str(n_iter).zfill(8) + "/" + dsopts["h5path"]),
            )

    return dset


def require_dataset_from_dsopts(
    group,
    dsopts,
    shape=None,
    dtype=None,
