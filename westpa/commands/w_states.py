...
from westpa.states import BasisState, TargetState
        ...
        if work_manager.is_master:
            ...
            if args.mode == "show":
                ...
                if basis_states:
                    bstate_file = (
                        sys.stdout
                        if not args.bstate_file
                        else open(args.bstate_file, "wt")
                    )
                    bstate_file.write(
                ...
                if target_states:
                    tstate_file = (
                        sys.stdout
                        if not args.tstate_file
                        else open(args.tstate_file, "wt")
                    )
                    tstate_file.write(
                ...
                if (seg_index["status"] == Segment.SEG_STATUS_COMPLETE).any():
                    print(
                        "Iteration {:d} has completed segments; applying new states to iteration {:d}".format(
                            n_iter, n_iter + 1
                        )
                    )
                    n_iter += 1
                ...
                if args.bstates:
                    for bstate_str in args.bstates:
                        ...
                        basis_states.append(
                            BasisState(
                                label=label, probability=probability, auxref=auxref
                            )
                        )
