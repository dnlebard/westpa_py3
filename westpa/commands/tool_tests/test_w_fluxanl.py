...
class Test_W_Fluxanl_Args(CommonToolTest):
    ...
    def check_output(self, **kwargs):
        ...
        with h5py.File(self.outfile) as f:
            assert "target_flux" in list(
                f.keys()
            ), "'target flux' group not in output file"
            target_group = f["target_flux"]
            assert "index" in list(
                target_group.keys()
            ), "'index' group not in output file"
            ...
            if self.mean_flux is not None:
                ...
                ), "Actual mean flux ({}) is different than expected ({})".format(
                    mean_flux, self.mean_flux
                )
            else:
            ...
            ), "Actual alpha ({}) does not match expected ({})".format(
            ...
            assert (
                expected_acalpha == actual_acalpha
            ), "Actual autocorrel alpha ({}) does not match expected ({})".format(
                actual_alpha, expected_alpha
            )

            expected_nsets = kwargs["nsets"] or 10 ** (
                math.ceil(-math.log10(expected_alpha)) + 1
            )
            actual_nsets = index_group.attrs["mcbs_n_sets"]

            assert (
                expected_nsets == actual_nsets
            ), "Actual n_sets ({}) does not match expected ({})".format(
...
class Test_W_Fluxanl_System(CommonToolTest):
    ...
    def test_alpha(self):
        ...
        default_alpha_tester.make_parser_and_process(
            args=["--alpha=0.05", "-o={}".format(self.outfile)]
        )
        low_alpha_tester.make_parser_and_process(
            args=["--alpha=0.01", "--autocorrel-alpha=0.05", "-o={}".format(out_low)]
        )
        high_alpha_tester.make_parser_and_process(
            args=["--alpha=0.20", "--autocorrel-alpha=0.05", "-o={}".format(out_high)]
        )
        ...
        ci_len_default = (
            data_default["mean_flux_ci_ub"][0] - data_default["mean_flux_ci_lb"][0]
        )
        ...
        assert (
            data_default["mean_flux"][0]
            == data_low["mean_flux"][0]
            == data_high["mean_flux"][0]
