...
"""
        ...
        for n_iter in range(1, self.data_manager.current_iteration):
            ...
            recycled_seg_ids = all_seg_ids[
                seg_index[:]["endpoint_type"] == Segment.SEG_ENDPOINT_RECYCLED
            ]
            ...
            final_pcoords = self.get_pcoord_dataset(n_iter)[
                recycled_seg_ids, pcoord_len - 1, :
            ]
            ...
            for (ipc, seg_id) in enumerate(recycled_seg_ids):
                self.output_file.write(
                    "%8d    %8d    %20.14g"
                    % (n_iter, seg_id, seg_index[seg_id]["weight"])
                )
                fields = [""]
