...
"""
    ...
    def process_args(self, args):
        ...
        self.output_file = (
            open(args.output_file, "wt") if args.output_file else sys.stdout
        )
    ...
    def go(self):
        ...
        for (_seg_id, segment) in enumerate(segments):
            parents_str = (
                "[" + ", ".join(map(str, sorted(segment.wtg_parent_ids))) + "]"
            )
            init_pcoord_str = (
                "["
                + ", ".join(
            ...
            if self.print_pcoords:
                self.output_file.write(
                    pcoord_lines.format(
                        init_pcoord=init_pcoord_str, final_pcoord=final_pcoord_str
                    )
                )
