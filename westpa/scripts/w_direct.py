...
"""
    ...
    def open_files(self):
        self.output_file = h5io.WESTPAH5File(
            self.output_filename, "a", creating_program=True
        )
        h5io.stamp_creator_data(self.output_file)
        self.assignments_file = h5io.WESTPAH5File(
            self.assignments_filename, "r"
...
"""
    ...
    def w_kinavg(self):
        ...
        avg_conditional_fluxes = self.run_calculation(
            eval_block=_2D_simple_eval_block,
        ...
        rate_evol = self.run_calculation(
            eval_block=_rate_eval_block, name="Rate Evolution", dim=2, **submit_kwargs
        )
        self.output_file.replace_dataset(
        ...
        self.output_file.replace_dataset(
            "target_flux_evolution", data=rate_evol, shuffle=True, compression=9
        )
...
"""
        ...
        try:
            for iiter, n_iter in enumerate(range(self.start_iter, self.stop_iter)):
                ...
                accumulate_state_populations_from_labeled(
                    labeled_pops, state_map, iter_state_pops, check_state_map=False
                )
    ...
    def w_stateprobs(self):
        ...
        color_evol_avg = self.run_calculation(
            name="Color Probability Evolution", dim=1, do_averages=True, **submit_kwargs
        )
        self.output_file.replace_dataset(
            "avg_color_probs", data=color_evol_avg[1], shuffle=True, compression=9
        )

        submit_kwargs["dataset"] = {"dataset": state_pops}
        state_evol_avg = self.run_calculation(
            name="State Probability Evolution", dim=1, do_averages=True, **submit_kwargs
        )
        self.output_file.replace_dataset(
            name="avg_state_probs", data=state_evol_avg[1], shuffle=True, compression=9
        )
        ...
        submit_kwargs["dataset"] = {"dataset": pops}
        pop_evol = self.run_calculation(
            name="Color Probability Evolution", dim=1, **submit_kwargs
        )
        self.output_file.replace_dataset(
            "color_prob_evolution", data=pop_evol, shuffle=True, compression=9
        )

        submit_kwargs["dataset"] = {"dataset": state_pops}
        pop_evol = self.run_calculation(
            name="State Probability Evolution", dim=1, **submit_kwargs
        )
        self.output_file.replace_dataset(
            name="state_pop_evolution", data=pop_evol, shuffle=True, compression=9
        )
