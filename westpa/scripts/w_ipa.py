...
class WIPI(WESTParallelTool):
    ...
    def add_args(self, parser):
        ...
        rgroup.add_argument(
            "--f", "-f", dest="extra", default="blah", help=argparse.SUPPRESS
        )
    ...
    def process_args(self, args):
        ...
        for ischeme, scheme in enumerate(self.__settings["analysis_schemes"]):
            if (
                self.__settings["analysis_schemes"][scheme]["enabled"]
    ...
    def analysis_structure(self):
        ...
        for scheme in self.__settings["analysis_schemes"]:
            if self.__settings["analysis_schemes"][scheme]["enabled"]:
                if not self.work_manager.running:
                ...
                try:
                    if (
                        self.__settings["analysis_schemes"][scheme]["postanalysis"]
                        or self.__settings["postanalysis"]
                    ):
                ...
                for name in analysis_files:
                    ...
                    if self.reanalyze:
                        ...
                        try:
                            ...
                            arg_hash = self.__analysis_schemes__[scheme][name].attrs[
                                "arg_hash"
                            ]
                            if name == "assign":
                    ...
                    if True:
                        if name == "assign":
                            ...
                            new_hash = self.hash_args(
                                ...
                                extra=[
                                    int(self.niters),
                                    codecs.encode(
                                        pickle.dumps(assign.binning.mapper), "base64"
                                    ),
                            ...
                            if self.debug_mode:
                                print(
                                    "{:<10}: old hash, new hash -- {}, {}".format(
                                        name, arg_hash, new_hash
                                    )
                                )
                            if not self.ignore_hash and (
                                arg_hash != new_hash or self.reanalyze
                            ):
                                ...
                                try:
                                    ...
                                    pass
                                print(
                                    "Reanalyzing file {}.h5 for scheme {}.".format(
                                        name, scheme
                                    )
                                )
                                # reanalyze_kinetics = True
                        ...
                        if name == "direct" or name == "reweight":
                            assignment_file = self.__analysis_schemes__[scheme][
                                "assign"
                            ]
                            if name == "direct":
                            ...
                            # if arg_hash != new_hash or self.reanalyze == or reanalyze_kinetics == True:
                            if self.debug_mode:
                                print(
                                    "{:<10}: old hash, new hash -- {}, {}".format(
                                        name, arg_hash, new_hash
                            ...
                            if not self.ignore_hash and (
                                arg_hash != new_hash or reanalyze_kinetics
                            ):
                                try:
                                    os.remove(os.path.join(path, "{}.h5".format(name)))
                                except Exception:
    ...
    @property
    def reweight(self):
        if self.__settings["analysis_schemes"][str(self.scheme)]["postanalysis"]:
    ...
    def scheme(self):
        ...
        if self._scheme is None:
            self._scheme = WIPIScheme(
                scheme=self.__analysis_schemes__,
    ...
    def scheme(self, scheme):
        ...
        if (
            ...
            print(
                "Scheme cannot be changed to scheme: {}; it is not enabled!".format(
                    scheme
                )
            )
    ...
    def trace(self, seg_id):
        ...
        if seg_id >= self.current.walkers:
            print(
                "Walker seg_id # {} is beyond the max count of {} walkers.".format(
                    seg_id, self.current.walkers
                )
            )
            return 1
        ...
        with pi:
            pi.new_operation(
                "Tracing scheme:iter:seg_id {}:{}:{}".format(
                    self.scheme, self.iteration, seg_id
                ),
                self.iteration,
            )
            current = {
            ...
            for iter in reversed(list(range(1, self.iteration + 1))):
                ...
                particles = self.data_reader.data_manager.get_iter_summary(int(iter))[
                    "n_particles"
                ]
            ...
            if isinstance(value, str):
                ...
                try:
                    ...
                    for key in list(self.__dict__["raw"]["auxdata"].keys()):
                        current["auxdata"][key] = self.__dict__["raw"]["auxdata"][key][
                            value
                        ]
                except Exception:
                    pass
                current = WIPIDataset(
                    current, "Segment {} in Iter {}".format(value, self.iteration)
                )
                return current
    ...
    def __get_children__(self):
        ...
        if self.iteration == self.niters:
            print(
                "Currently at iteration {}, which is the max.  There are no children!".format(
                    self.iteration
                )
            )
            return 0
        ...
        for seg_id in range(0, self.current.walkers):
            ...
            if len(children) == 0:
                ...
                future["states"].append(
                    self.assign["trajlabels"][value - 1, children, :]
                )
                future["bins"].append(
                    self.assign["assignments"][value - 1, children, :]
                )
        return future
    ...
    def go(self):
        ...
        print("")
        print(
            "Welcome to w_ipa (WESTPA Interactive Python Analysis) v. {}!".format(
                w.version
            )
        )
        print(
        ...
        try:
            print(
                "Your current scheme, system and iteration are : {}, {}, {}".format(
                    w.scheme, os.getcwd(), w.iteration
                )
            )
        except Exception:
            pass
    ...
    def introduction(self):
        ...
        """.format(
            ...
            w=self.__format_keys__(
                self.__dir__(), offset=8, max_length=0, split="", prepend="w."
            ),
        )
        print(help_string)
