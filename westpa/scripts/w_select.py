...
def _find_matching_segments(west_datafile_name, n_iter, predicate, invert=False):
    ...
    with h5io.WESTPAH5File(west_datafile_name, "r") as west_datafile:
        ...
        matchvec = sorted(
            numpy.fromiter(matching_ids, dtype=seg_id_dtype, count=len(matching_ids))
        )
...
"""
    ...
    def go(self):
        ...
        output_file.create_dataset(
            "n_iter", dtype=n_iter_dtype, data=list(range(iter_start, iter_stop))
        )
        current_seg_count = 0
        seg_count_ds = output_file.create_dataset(
            "n_segs", dtype=numpy.uint, shape=(iter_count,)
        )
        ...
        with pi:
            ...
            for future in self.work_manager.submit_as_completed(
                (
                    (
                        ...
                        (
                            self.data_reader.we_h5filename,
                            n_iter,
                            self.predicate,
                            self.invert,
                        ),
                ...
                if n_matches:
                    ...
                    weights_ds[
                        n_iter - iter_start, :n_matches
                    ] = self.data_reader.get_iter_group(n_iter)["seg_index"]["weight"][
                        sorted(matching_ids)
                    ]
                del matching_ids
                pi.progress += 1
            ...
            if self.include_ancestors:
                pi.new_operation(
                    "Tracing ancestors of matching segments", extent=iter_count
                )
                from_previous = set()
                ...
                for n_iter in range(iter_stop - 1, iter_start - 1, -1):
                    ...
                    if n_matches > 0:
                        ...
                        weights_ds[iiter, :n_matches] = self.data_reader.get_iter_group(
                            n_iter
                        )["seg_index"]["weight"][sorted(matching_ids)]
                        parent_ids = self.data_reader.get_iter_group(n_iter)[
                            "seg_index"
                        ]["parent_id"][sorted(matching_ids)]
                        from_previous.update(
                            parent_id for parent_id in parent_ids if parent_id >= 0
