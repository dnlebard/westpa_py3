...
from westpa.westtools.binning import write_bin_info
...
"""
    ...
    def cmd_info(self):
        ...
        write_bin_info(
            mapper, assignments, weights, n_target_states, detailed=self.args.detail
        )
    ...
    def cmd_rebin(self):
        ...
        last_iter_segments = data_manager.get_segments(
            self.n_iter - 1, load_pcoords=False, load_auxdata=False
        )
        ...
        we_driver.used_initial_states = {
            state.state_id: state for state in used_initial_states
        }
        ...
        assignments = numpy.fromiter(
            we_driver.next_iter_assignments, dtype=int, count=len(weights)
        )
        write_bin_info(
            mapper, assignments, weights, n_target_states, detailed=self.args.detail
        )
        ...
        if self.args.confirm:
            data_manager.prepare_iteration(
                self.n_iter, list(we_driver.next_iter_segments)
            )

            # manually update endpoint statuses only
            endpoint_types = sorted(
                [
                    (segment.seg_id, segment.endpoint_type)
