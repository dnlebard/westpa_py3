...
ds_uncorrel = ds_correl = numpy.loadtxt(
    "/home/mzwier/flexible-p53-MDM2-1d-rmsd/avgflux_1_blocked.txt", usecols=[2]
)
...
print("naive MCBS:")
uncorrel_mean, uncorrel_lb, uncorrel_ub = mcbs_ci(
    ds_uncorrel, numpy.mean, 0.05, n_sets=n_sets, sort=numpy.msort
)
correl_mean, correl_lb, correl_ub = mcbs_ci(
    ds_correl, numpy.mean, 0.05, n_sets=n_sets, sort=numpy.msort
)
...
correl_mean, correl_lb, correl_ub, k_ = mcbs_ci_correl(
    ds_correl, numpy.mean, 0.05, n_sets=n_sets, subsample=numpy.mean
)
