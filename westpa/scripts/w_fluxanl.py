...
target_index_dtype = numpy.dtype(
    [
...
def _extract_fluxes_fileversion_lt_7(iter_start, iter_stop, data_manager):
    ...
    fluxdata = {
        itarget: numpy.zeros((iter_count,), dtype=fluxentry_dtype)
...
def _extract_fluxes_fileversion_7(iter_start, iter_stop, data_manager):
    ...
    for iiter, n_iter in enumerate(range(iter_start, iter_stop)):
        ...
        try:
            new_weight_index = data_manager.get_iter_group(n_iter + 1)["new_weights"][
                "index"
            ]
        except KeyError:
        ...
        for tstate in target_states:
            try:
                ...
                target_info = by_target[tstate.label] = numpy.zeros(
                    (iter_count,), dtype=fluxentry_dtype
                )
            ...
            if recycle_count:
                ...
                target_info["flux"][iiter] = new_weight_index[recycled_from_tstate][
                    "weight"
                ].sum()
    ...
    for target_label in by_target:
        ...
        by_target[target_label] = fluxdata[
            numpy.searchsorted(fluxdata["count"], [0])[0] :
        ]
...
"""
    ...
    def calc_store_flux_data(self):
        ...
        fluxdata = extract_fluxes(
            self.iter_range.iter_start, self.iter_range.iter_stop, self.data_reader
        )

        # Create a group to store data in
        output_group = h5io.create_hdf5_group(
            self.output_h5file, "target_flux", replace=False, creating_program=self.prog
        )
    ...
    def calc_evol_flux(self):
        westpa.rc.pstatus(
            "Calculating cumulative evolution of flux confidence intervals every {} iteration(s)".format(
                self.evol_step
            )
        )

        for itarget, (target_label, target_fluxdata) in enumerate(
            self.fluxdata.items()
        ):
            fluxes = target_fluxdata["flux"]
            ...
            for iblock in range(n_blocks):
                ...
                (
                    cis[iblock]["expected"],
                    cis[iblock]["ci_lbound"],
                    cis[iblock]["ci_ubound"],
                ) = (avg, ci_lb, ci_ub)
