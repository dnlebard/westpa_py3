...
from westpa.states import InitialState
from westpa.data_manager import (
    weight_dtype,
    n_iter_dtype,
    seg_id_dtype,
    utime_dtype,
...
class Trace:
    ...
    def __init__(
        self, summary, endpoint_type, basis_state, initial_state, data_manager=None
    ):
    ...
    def from_data_manager(cls, n_iter, seg_id, data_manager=None):
        ...
        first_segment = Segment(
            n_iter=first_iter, seg_id=first_seg_id, parent_id=first_parent_id
        )

        seginfo.reverse()

        summary_dtype = numpy.dtype(
            [
                ("n_iter", n_iter_dtype),
                ("seg_id", seg_id_dtype),
                ("weight", weight_dtype),
        ...
        try:
            ...
            initial_state = InitialState(
                istate_id, None, iter_created=0, pcoord=istate_pcoord
            )
    ...
    def get_segment_data_slice(
        ...
        if index_data is not None:
            ...
            if not iter_prec:
                iter_prec = datafile.attrs.get(
                    "west_iter_prec", self.data_manager.default_iter_prec
                )
            igname_tail = "iter_{:0{iter_prec:d}d}".format(
                int(n_iter), iter_prec=int(iter_prec)
            )
            try:
    ...
    def trace_timepoint_dataset(self, dsname, slice_=None, auxfile=None, index_ds=None):
        ...
        first_n_iter, first_seg_id = (
            self.summary[0]["n_iter"],
            self.summary[0]["seg_id"],
        )
        first_iter_data = get_data_slice(
            datafile,
            dsname,
            first_n_iter,
            first_seg_id,
        ...
        tracedata = numpy.empty(
            (length,) + first_iter_data.shape[1:], dtype=first_iter_data.dtype
        )
        ...
        for iseg, summary_item in enumerate(self.summary[1:]):
            ...
            seg_data = get_data_slice(
                datafile, dsname, n_iter, seg_id, other_sl, index_data, iter_prec
            )
...
"""
    ...
    def process_args(self, args):
        ...
        self.endpoints = [
            list(map(int, endpoint.split(":"))) for endpoint in args.endpoints
        ]
    ...
    def go(self):
        ...
        try:
            trajs_group = h5io.create_hdf5_group(
                self.output_file,
                "trajectories",
                replace=False,
        ...
        for n_iter, seg_id in self.endpoints:
            ...
            trace = Trace.from_data_manager(
                n_iter, seg_id, self.data_reader.data_manager
            )
...
""".format(
        ...
        pcoord_str = "    ".join(
            pcoord_formats.get(pcfield.dtype.str[1:], "%s") % pcfield
