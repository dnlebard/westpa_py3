...
class CommonPloterrs(WESTSubcommand):
    ...
    def add_args(self, parser):
        ...
        pogroup.add_argument(
            "--xlabel", help="""Use XLABEL for the x-axis label. (Default: varies.)"""
        )
        pogroup.add_argument(
            "--ylabel", help="""Use YLABEL for the y-axis label. (Default: varies.)"""
        )
        pogroup.add_argument(
            "--title", help="""Use TITLE for the plot title. (Default: varies.)"""
        )
        pogroup.add_argument(
            "--terminal",
            "-t",
    ...
    def parse_range(self, rangespec):
        try:
            ...
            raise ValueError(
                "invalid range specification {!r}: {!s}".format(rangespec, e)
            )
...
"""
    ...
    def plot_flux(self, istate):
        ...
        output_filename = (
            self.flux_output_pattern.format(**subdict)
            if self.flux_output_pattern
            else None
        )

        title = (
            self.title if self.title is not None else 'Flux into state "{state_label}"'
        )
    ...
    def plot_rate(self, istate, jstate):
        ...
        output_filename = (
            self.rate_output_pattern.format(**subdict)
            if self.rate_output_pattern
            else None
        )
        ...
        title = (
            ...
            else 'Rate from state "{istate_label}" to state "{fstate_label}"'
        )
        title = title.format(**subdict)
    ...
    def go(self):
        ...
        if self.interface == "matplotlib":
            ...
            plotter = Plotter(
                self.kinavg_file, "rate_evolution", iteration=-1, interface="text"
            )
            for istate in range(nstates):
                for jstate in range(nstates):
                    if istate != jstate:
...
"""
    ...
    def plot_pop(self, istate):
        ...
        output_filename = (
            self.pop_output_pattern.format(**subdict)
            if self.pop_output_pattern
            else None
        )
        ...
        title = (
            self.title
            if self.title is not None
            else 'Population in state "{state_label}"'
        )
        title = title.format(**subdict)
    ...
    def plot_color(self, istate):
        ...
        output_filename = (
            self.color_output_pattern.format(**subdict)
            if self.color_output_pattern
            else None
        )
        ...
        title = (
            self.title
            if self.title is not None
            else 'Population in ensemble "{state_label}"'
        )
        title = title.format(**subdict)
