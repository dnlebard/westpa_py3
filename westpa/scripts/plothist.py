...
import re
import os
from westpa.westtools import WESTMasterCommand, WESTSubcommand
import numpy
import h5py
...
class PlotHistBase(WESTSubcommand):
    ...
    def add_args(self, parser):
        ...
        ogroup.add_argument(
            "--hdf5-output", help="""Store plot data in the HDF5 file HDF5_OUTPUT."""
        )
        ogroup.add_argument(
            "--plot-contour",
            dest="plot_contour",
        ...
        pgroup.add_argument(
            "--title", dest="title", help="Include TITLE as the top-of-graph title"
        )
        ...
        if args.enerzero:
            ...
            if lenerzero not in ("min", "max"):
                try:
                    ...
                    raise ValueError(
                        "invalid energy zero point {!r}".format(args.enerzero)
                    )
            else:
    ...
    def parse_dimspec(self, dimspec):
        ...
        try:
            ...
            raise ValueError(
                "invalid dimension specification {!r}: {!r}".format(dimspec, e)
            )
        return dimdata
    ...
    def parse_range(self, rangespec):
        try:
            ...
            raise ValueError(
                "invalid range specification {!r}: {!r}".format(rangespec, e)
            )
    ...
    def _do_1d_output(self, hist, idim, midpoints):
        ...
        if self.text_output_filename:
            with textio.NumericTextOutputFormatter(
                self.text_output_filename
            ) as output_file:
                output_file.write_header(
                    "source data: {} dimension {}".format(
...
"""
    ...
    def do_instant_plot_2d(self):
        ...
        self._do_2d_output(
            hist, [idim0, idim1], [midpoints_0, midpoints_1], [binbounds_0, binbounds_1]
        )
...
"""
    ...
    def do_average_plot_2d(self):
        ...
        for iiter in range(iiter_start, iiter_stop):
            iter_hist = sum_except_along(
                self.input_h5["histograms"][iiter], [idim0, idim1]
            )
            if iiter == iiter_start:
                hist = iter_hist
            else:
...
"""
    ...
    def process_args(self, args):
        ...
        log.info(
            "using data for iterations {} -- {} with a step of {}".format(
    ...
    def go(self):
        ...
        blocked_hists = numpy.zeros(
            (nblocks, hists_ds.shape[1 + idim]), dtype=hists_ds.dtype
        )

        for iblock, istart in enumerate(
            range(iiter_start, iiter_start + nblocks * self.iter_step, self.iter_step)
        ):
