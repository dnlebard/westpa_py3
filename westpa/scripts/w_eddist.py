...
class DurationDataset:
    ...
    def get_iter_data(self, n_iter):
        try:
            ...
            raise ValueError(
                "Iteration {} is not within the iteration range".format(n_iter)
            )
        nsegs = dset.shape[0]
        if nsegs == 0:
            return None
...
def _remote_bin_iter(
    iiter, n_iter, dsspec, wt_dsspec, initpoint, binbounds, ignore_out_of_range
):
...
"""
    ...
    def go(self):
        ...
        with pi:
            self.duration = self.kinetics_file["durations"][
                self.iter_start - 1 : self.iter_stop - 1
            ]

            ##Only select transition events from specified istate to fstate
            mask = (self.duration["istate"] == self.istate) & (
                self.duration["fstate"] == self.fstate
            )

            self.duration_dsspec = DurationDataset(
                self.kinetics_file["durations"]["duration"], mask, self.iter_start
            )
            self.wt_dsspec = DurationDataset(
                self.kinetics_file["durations"]["weight"], mask, self.iter_start
            )
            ...
            iter_range = numpy.arange(
                self.iter_start,
                self.iter_stop,
                1,
                dtype=(numpy.min_scalar_type(self.iter_stop)),
            )
    ...
    def scan_data_range(self):
        ...
        self.progress.indicator.new_operation(
            "Scanning for data range", self.iter_stop - self.iter_start
        )
    ...
    def _construct_bins_from_bound_seqs(self, bins):
        ...
        for boundset in bins:
            ...
            if (numpy.diff(boundset) <= 0).any():
                raise ValueError(
                    "boundary set {!r} is not strictly monotonically increasing".format(
                        boundset
                    )
                )
            self.binbounds.append(boundset)
    ...
    def construct_histogram(self):
        ...
        binbounds = [
            numpy.require(boundset, self.dset_dtype, "C") for boundset in self.binbounds
        ]

        self.progress.indicator.new_operation(
            "Constructing histograms", self.iter_stop - self.iter_start
        )
        task_gen = (
            (
                _remote_bin_iter,
