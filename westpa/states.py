...
class BasisState:
    ...
    def states_from_file(cls, filename):
        ...
        for line in open(filename, "rt"):
            ...
            try:
                ...
                raise ValueError(
                    "invalid probability ({!r}) {} line {:d}".format(
                        fields[1], filename, lineno
                    )
                )
            ...
            states.append(
                cls(state_id=None, probability=probability, label=label, auxref=auxref)
            )
        return states
    ...
    def as_numpy_record(self):
        ...
        from westpa.data_manager import vstr_dtype, weight_dtype, seg_id_dtype
...
InitialState.istate_statuses.update(
    {
        _attr: getattr(InitialState, _attr)
        for _attr in dir(InitialState)
        if _attr.startswith("ISTATE_STATUS_")
    }
)
InitialState.istate_types.update(
    {
        _attr: getattr(InitialState, _attr)
        for _attr in dir(InitialState)
        if _attr.startswith("ISTATE_TYPE_")
    }
)
...
class TargetState:
    ...
    def states_to_file(cls, states, fileobj):
        ...
        fileobj.write(
            "# {:{max_label_len}s}    {:s}\n".format(
                "Label", "Pcoord", max_label_len=max_label_len - 2
            )
        )
        for state in states:
            ...
            fileobj.write(
                "{:{max_label_len}s}    {:s}\n".format(
                    state.label, pcoord_str, max_label_len=max_label_len
                )
            )
    ...
    def states_from_file(cls, statefile, dtype):
        ...
        return [
            cls(label=label, pcoord=pcoord)
            for label, pcoord in zip(labels, pcoord_values)
        ]
...
def pare_basis_initial_states(basis_states, initial_states, segments=None):
    ...
    if segments is not None:
        ...
        return_istates = set(
            istatemap[segment.initial_state_id]
