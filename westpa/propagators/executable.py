...
import numpy
import logging
from westpa.states import BasisState, InitialState

import westpa
from westpa.extloader import get_object
from westpa.yamlcfg import check_bool
from westpa.segment import Segment
from westpa.propagators import WESTPropagator
...
def pcoord_loader(fieldname, pcoord_return_filename, destobj, single_point):
    ...
    if pcoord.shape != expected_shape:
        raise ValueError(
            "progress coordinate data has incorrect shape {!r} [expected {!r}]".format(
...
class ExecutablePropagator(WESTPropagator):
    ...
    def __init__(self, rc=None):
        ...
        self.basis_state_ref_template = config[
            "west", "data", "data_refs", "basis_state"
        ]
        ...
        self.addtl_child_environ.update(
            {
                k: str(v)
                for k, v in (config["west", "executable", "environ"] or {}).items()
            }
        )
        ...
        for child_type in (
            ...
            self.exe_info[child_type]["environ"] = {
                k: str(v) for k, v in (child_info.get("environ") or {}).items()
            }
    ...
    def exec_child(
        self, executable, environ=None, stdin=None, stdout=None, stderr=None, cwd=None
    ):
    ...
    def exec_child_from_child_info(self, child_info, template_args, environ):
        ...
        return self.exec_child(
            ...
            environ=environ,
            cwd=self.makepath(child_info["cwd"], template_args)
            if child_info["cwd"]
            else None,
            stdin=self.makepath(child_info["stdin"], template_args)
            if child_info["stdin"]
            else os.devnull,
            stdout=self.makepath(child_info["stdout"], template_args)
            if child_info["stdout"]
            else None,
            stderr=self.makepath(child_info["stderr"], template_args)
    ...
    def update_args_env_basis_state(self, template_args, environ, basis_state):
        ...
        new_env = {
            self.ENV_BSTATE_ID: str(
                basis_state.state_id if basis_state.state_id is not None else -1
            ),
            self.ENV_BSTATE_DATA_REF: self.makepath(
                self.basis_state_ref_template, new_template_args
            ),
        }
    ...
    def update_args_env_initial_state(self, template_args, environ, initial_state):
        ...
        new_env = {
            self.ENV_ISTATE_ID: str(
                initial_state.state_id if initial_state.state_id is not None else -1
            ),
            self.ENV_ISTATE_DATA_REF: self.makepath(
    ...
    def update_args_env_segment(self, template_args, environ, segment):
        ...
        if segment.initpoint_type == Segment.SEG_INITPOINT_CONTINUES:
            ...
            if self.ENV_ISTATE_ID not in environ:
                self.update_args_env_initial_state(
                    template_args, environ, initial_state
                )
    ...
    def get_pcoord(self, state):
        ...
        try:
            ...
            if rc != 0:
                log.error(
                    "get_pcoord executable {!r} returned {}".format(
                        child_info["executable"], rc
                    )
                )
            ...
            try:
                ...
                log.warning(
                    "could not delete progress coordinate return file {!r}: {}".format(
                        rfname, e
                    )
                )
    ...
    def gen_istate(self, basis_state, initial_state):
        ...
        if rc != 0:
            log.error(
                "gen_istate executable {!r} returned {}".format(
                    child_info["executable"], rc
                )
            )
    ...
    def prepare_iteration(self, n_iter, segments):
        ...
        if child_info and child_info["enabled"]:
            try:
                ...
                log.warning(
                    "could not execute pre-iteration program {!r}: {}".format(
                        child_info["executable"], e
                    )
                )
            else:
                if rc != 0:
                    log.warning(
                        "pre-iteration executable {!r} returned {}".format(
                            child_info["executable"], rc
                        )
                    )
    ...
    def finalize_iteration(self, n_iter, segments):
        ...
        if child_info and child_info["enabled"]:
            try:
                ...
                log.warning(
                    "could not execute post-iteration program {!r}: {}".format(
                        child_info["executable"], e
                    )
                )
            else:
                if rc != 0:
                    log.warning(
                        "post-iteration executable {!r} returned {}".format(
                            child_info["executable"], rc
                        )
                    )
    ...
    def propagate(self, segments):
        ...
        for segment in segments:
            ...
            for dataset in self.data_info:
                ...
                if return_template:
                    return_files[dataset] = self.makepath(
                        return_template, self.template_args_for_segment(segment)
                    )
                ...
                addtl_env["WEST_{}_RETURN".format(dataset.upper())] = return_files[
                    dataset
                ]
            ...
            if rc == 0:
                ...
                log.error(
                    "child process for segment %d exited on signal %d (%s)"
                    % (segment.seg_id, -rc, SIGNAL_NAMES[-rc])
                )
                segment.status = Segment.SEG_STATUS_FAILED
                continue
            else:
                log.error(
                    "child process for segment %d exited with code %d"
                    % (segment.seg_id, rc)
                )
                segment.status = Segment.SEG_STATUS_FAILED
                continue
            ...
            for dataset in self.data_info:
                ...
                try:
                    ...
                    log.error(
                        "could not read {} from {!r}: {!r}".format(dataset, filename, e)
                    )
                    segment.status = Segment.SEG_STATUS_FAILED
                    break
                else:
                    if del_return_files[dataset]:
                        try:
                            ...
                            log.warning(
                                "could not delete {} file {!r}: {!r}".format(
                                    dataset, filename, e
                                )
                            )
                        else:
