...
from westpa.weed.ProbAdjustEquil import probAdjustEquil
...
class WEEDDriver:
    def __init__(self, sim_manager, plugin_config):
        ...
        self.do_reweight = check_bool(
            plugin_config.get("do_equilibrium_reweighting", False)
        ...
        if windowsize is not None:
            if isinstance(windowsize, float):
                ...
                if self.windowsize <= 0 or self.windowsize > 1:
                    raise ValueError(
                        "WEED parameter error -- fractional window size must be in (0,1]"
                    )
                ...
                raise ValueError(
                    "WEED parameter error -- invalid window size {!r}".format(
                        windowsize
                    )
                )
        log.info(
            "using window size of {!r} ({})".format(self.windowsize, self.windowtype)
        )
    ...
    def get_rates(self, n_iter, mapper):
        ...
        averager = RateAverager(
            mapper, self.system, self.data_manager, self.work_manager
        )
        averager.calculate(
            max(1, n_iter - eff_windowsize),
            n_iter + 1,
            self.rate_calc_n_blocks,
            self.rate_calc_queue_size,
        )
    ...
    def prepare_new_iteration(self):
        ...
        if we_driver.target_states and self.do_reweight:
            log.warning(
                "equilibrium reweighting requested but target states (sinks) present; reweighting disabled"
            )
            return
        ...
        with self.data_manager.lock:
            ...
            avg_populations_ds = weed_iter_group.create_dataset(
                "avg_populations", shape=(n_bins,), dtype=numpy.float64
            )
            unc_populations_ds = weed_iter_group.create_dataset(
                "unc_populations", shape=(n_bins,), dtype=numpy.float64
            )
            avg_flux_ds = weed_iter_group.create_dataset(
                "avg_fluxes", shape=(n_bins, n_bins), dtype=numpy.float64
            )
            unc_flux_ds = weed_iter_group.create_dataset(
                "unc_fluxes", shape=(n_bins, n_bins), dtype=numpy.float64
            )
            avg_rates_ds = weed_iter_group.create_dataset(
                "avg_rates", shape=(n_bins, n_bins), dtype=numpy.float64
            )
            unc_rates_ds = weed_iter_group.create_dataset(
                "unc_rates", shape=(n_bins, n_bins), dtype=numpy.float64
            )
        ...
        rc.pstatus(
            "Calculating equilibrium reweighting using window size of {:d}".format(
                self.eff_windowsize
            )
        )
        ...
        assert (
            ~((orig_binprobs > 0) & (binprobs == 0))
        ).all(), "populated bin reweighted to zero probability"

        # Check to see if reweighting has set zero bins to nonzero probability (may happen)
        z2nz_mask = (orig_binprobs == 0) & (binprobs > 0)
        if (z2nz_mask).any():
            rc.pstatus(
                "Reweighting would assign nonzero probability to an empty bin; not reweighting this iteration."
            )
            rc.pstatus(
                "Empty bins assigned nonzero probability: {!s}.".format(
                    numpy.array_str(numpy.arange(n_bins)[z2nz_mask])
                )
            )
        else:
