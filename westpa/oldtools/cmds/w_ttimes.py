...
from westpa.oldtools.aframe import (
    WESTAnalysisTool,
    BinningMixin,
    WESTDataReaderMixin,
    IterRangeMixin,
...
class WTTimesBase:
    ...
    def gen_stats(self):
        ...
        if not self.bf_mode:
            ...
            transdat_in_range = (transdat_niter >= self.first_iter) & (
                transdat_niter <= self.last_iter
            )
        ...
        for ibin in self.analysis_initial_bins:
            ...
            for fbin in self.analysis_final_bins:
                ...
                trans_ifbins = numpy.extract(
                    trans_ibin["final_bin"] == fbin, trans_ibin
                )
                ...
                durations[ibin, fbin]["expectation"] = (
                    numpy.average(trans_durations, weights=trans_weights) * dt
                )
                fpts[ibin, fbin]["expectation"] = (
                    numpy.average(trans_fpts, weights=trans_weights) * dt
                )
                ...
                for iset in range(n_sets):
                    ...
                    syn_avg_durations[iset] = (
                        numpy.average(syn_durations, weights=syn_weights) * dt
                    )
                    syn_avg_fpts[iset] = (
                        numpy.average(syn_fpts, weights=syn_weights) * dt
                    )
            ...
            if filename:
                if array is None:
                    try:
                        ...
                        westpa.rc.pstatus(
                            "{} data not found in {}".format(title, self.anal_h5name)
                        )
                        continue
...
""".format(
        ...
        for ibin in self.analysis_initial_bins:
            for fbin in self.analysis_final_bins:
                ...
                outfile.write(
                    format_2d.format(
                        *list(map(float, (mean, lb, ub, ciwidth, relciwidth, symmerr))),
                        ibin=ibin,
                        fbin=fbin,
                        mw=max_ibin_width
                    )
...
class WTTimesBF(
    ...
    description = (
        "Trace one or more brute force trajectories and report on transition kinetics."
    )
