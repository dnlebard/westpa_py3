...
class WEDriver:
    ...
    def assign(self, segments, initializing=False):
        ...
        all_pcoords = numpy.empty(
            (2, len(segments), self.system.pcoord_ndim), dtype=self.system.pcoord_dtype
        )
        ...
        for segment in segments:
            if segment is gparent_seg:
                ...
                if segment.parent_id >= 0:
                    self._parent_map[
                        segment.parent_id
                    ].endpoint_type = Segment.SEG_ENDPOINT_MERGED
                else:
    ...
    def populate_initial(self, initial_states, weights, system=None):
        ...
        segments = []
        for (seg_id, (initial_state, weight)) in enumerate(
            zip(initial_states, weights)
        ):
