...
class Message:
    ...
    NAK = "no"
    IDENTIFY = (
        "identify"  # Two-way identification (a reply must be an IDENTIFY message)
    )
    ...
    def coalesce_announcements(cls, messages):
        ...
        log.debug(
            "coalesced {} announcements into {}".format(len(messages), len(coalesced))
        )
        return coalesced
...
class Result:
    ...
    def __repr__(self):
        return "<{} {task_id!s} ({})>".format(
            self.__class__.__name__,
            "result" if self.exception is None else "exception",
            **self.__dict__
        )
...
class ZMQCore:
        ...
        while cls._ipc_endpoints_to_delete:
            ...
            try:
                ...
                if e.errno != errno.ENOENT:
                    log.debug(
                        "could not unlink IPC endpoint {!r}: {}".format(socket_path, e)
                    )
            else:
    ...
    def __init__(self):
        ...
        self.node_description = "{!s} on {!s} at PID {:d}".format(
            self.__class__.__name__, socket.gethostname(), os.getpid()
        )

        self.validation_fail_action = "exit"  # other options are 'raise' and 'warn'

        self.log = logging.getLogger(
            __name__ + "." + self.__class__.__name__ + "." + str(self.node_id)
        )
        ...
        if os.environ.get("WWMGR_ZMQ_DEBUG_MESSAGES", "n").upper() in {
            "Y",
            "YES",
            "1",
            "T",
    ...
    def validate_message(self, message):
        ...
        if (
            ...
            and message.master_id is not None
            and message.master_id != self.master_id
        ):
    ...
    def recv_all(self, socket, flags=0, validate=True):
        ...
        while True:
            try:
                messages.append(
                    self.recv_message(socket, flags | zmq.NOBLOCK, validate)
                )
            except zmq.Again:
                return messages
    ...
    def send_reply(
        self, socket, original_message, reply=Message.ACK, payload=None, flags=0
    ):
        ...
        assert (
            original_message.worker_id is not None
        )  # should have been caught by validation prior to this
    ...
    def shutdown_handler(self, signal=None, frame=None):
        if signal is None:
            ...
            self.log.info(
                "shutting down on signal {!s}".format(signames.get(signal, signal))
            )
...
def shutdown_process(process, timeout=1.0):
    ...
    if process.is_alive():
        ...
        log.debug(
            "process {:d} terminated with code {:d}".format(
...
class IsNode:
    def __init__(self, n_local_workers=None):
        ...
        if n_local_workers:
            ...
            self.local_workers = [
                ZMQWorker(self.local_rr_endpoint, self.local_ann_endpoint)
                for _n in range(n_local_workers)
            ]
        else:
        ...
        self.local_worker_processes = [
            multiprocessing.Process(target=worker.startup, args=(n,))
    ...
    def write_host_info(self, filename=None):
        ...
        with open(filename, "wt") as infofile:
            info = {}
            info["rr_endpoint"] = re.sub(
                r"\*", hostname, self.downstream_rr_endpoint or ""
            )
            info["ann_endpoint"] = re.sub(
                r"\*", hostname, self.downstream_ann_endpoint or ""
            )
