...
def normhistnd(hist, binbounds):
    ...
    if ndim != len(binbounds):
        raise ValueError(
            "shape of histogram [{!r}] does not match bin boundary sets (there are {})".format(
                hist.shape, len(binbounds)
            )
        )
    ...
    if ndim == 1:
        ...
        assert outers.shape == hist.shape, "hist shape {} != outers shape {}".format(
            hist.shape, outers.shape
        )
