...
class TestWEDriver:
    ...
    def test_assign(self):
        ...
        assert (
            self.we_driver.flux_matrix == numpy.array([[0.0, 0.5], [0.5, 0.0]])
        ).all()
    ...
    def test_passthrough(self):
        ...
        assert (
            abs(sum(seg.weight for seg in self.we_driver.next_iter_segments) - 1.0)
            < 8 * EPS
        )
        ...
        for segment in self.we_driver.next_iter_segments:
            ...
            # is weight correct?
            assert (
                segment.weight == 0.125
            )  # rigorous floating point comparison okay here because no math was done
    ...
    def test_split_by_weight(self):
        ...
        assert (
            abs(sum(seg.weight for seg in self.we_driver.next_iter_binning[0]) - 0.25)
            < 4 * EPS
        )
        assert (
            abs(sum(seg.weight for seg in self.we_driver.next_iter_binning[1]) - 0.75)
            < 4 * EPS
        )
    ...
    def test_merge_by_weight(self):
        ...
        for _iround in range(nrounds):
            ...
            assert (
                segments[newseg.parent_id].endpoint_type
                == Segment.SEG_ENDPOINT_CONTINUES
            )
            assert (
                segments[~newseg.parent_id].endpoint_type == Segment.SEG_ENDPOINT_MERGED
            )
    ...
    def test_recycle(self):
        ...
        assert (
            abs(sum(seg.weight for seg in self.we_driver.next_iter_binning[0]) - 1.0)
            < 4 * EPS
        )
        assert numpy.allclose(
            [seg.weight for seg in self.we_driver.next_iter_binning[0]],
