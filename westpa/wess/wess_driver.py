...
from westpa.wess.ProbAdjust import prob_adjust
...
class WESSDriver:
    ...
    def prepare_new_iteration(self):
        ...
        n_bins = len(bins)
        pstatus("Averaging rates")
        averager = self.get_rates(n_iter, mapper)
        binprobs = numpy.fromiter(
            map(operator.attrgetter("weight"), bins), dtype=numpy.float64, count=n_bins
        )
        ...
        if self.write_matrices:
            ...
            with self.data_manager.lock:
                ...
                wess_iter_group.create_dataset(
                    "avg_populations", data=averager.average_populations, compression=4
                )
                wess_iter_group.create_dataset(
                    "unc_populations", data=averager.stderr_populations, compression=4
                )
                wess_iter_group.create_dataset(
                    "avg_fluxes", data=averager.average_flux, compression=4
                )
                wess_iter_group.create_dataset(
                    "unc_fluxes", data=averager.stderr_flux, compression=4
                )
                wess_iter_group.create_dataset(
                    "avg_rates", data=averager.average_rate, compression=4
                )
                wess_iter_group.create_dataset(
                    "unc_rates", data=averager.stderr_rate, compression=4
                )

        pstatus(
            "Calculating reweighting using window size of {:d}".format(
                self.eff_windowsize
            )
        )
        ...
        for target_region in target_regions:
            if (
                target_region in oldindex
            ):  # it is possible that the target region was removed (ie if no recycling has occurred)
