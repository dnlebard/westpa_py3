...
from westpa.yamlcfg import YAMLConfig
from westpa.yamlcfg import YAMLSystem
import westpa
...
def bins_from_yaml_dict(bin_dict):
    ...
    try:
        ...
        raise KeyError(
            "unknown bin mapper type {!r} in config file {!r}".format(typename)
        )
...
def parsePCV(pc_str):
    ...
    namespace = {"math": math, "numpy": numpy, "inf": float("inf")}
...
class WESTRC:
    ...
    def __init__(self):
        self.verbosity = None
        self.rcfile = (
            os.environ.get(self.ENV_RUNTIME_CONFIG) or self.RC_DEFAULT_FILENAME
        )
    ...
    def add_args(self, parser):
        ...
        group.add_argument(
            "--version", action="version", version="WEST version %s" % westpa.version
        )
    ...
    def process_args(self, args, config_required=True):
        ...
        self.config_logging()
        self.config["args"] = {
            k: v for k, v in args.__dict__.items() if not k.startswith("_")
        }
        self.process_config()
    ...
    def process_config(self):
        ...
        sys.path.extend(
            self.config.get_pathlist(["west", "drivers", "module_path"], [])
        )
        try:
...
"""
    ...
    def new_sim_manager(self):
        ...
        if drivername.lower() == "default":
            from westpa.sim_manager import WESimManager
    ...
    def update_from_yaml(self, init_system, system_dict):
        ...
        for key, value in system_dict.items():
            if key == "pcoord_ndim":
                ...
                self.overwrite_option(
                    init_system, "bin_mapper", bins_from_yaml_dict(value)
                )
        ...
        try:
            ...
            if hasattr(trgt_cnt, "__iter__"):
                assert (
                    len(trgt_cnt) == init_system.bin_mapper.nbins
                ), "Count iterable size doesn't match the number of bins"
                trgt_cnt_arr = trgt_cnt
            else:
                assert trgt_cnt == numpy.int(
