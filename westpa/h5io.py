...
def resolve_filepath(
    path, constructor=h5py.File, cargs=None, ckwargs=None, **addtlkwargs
):
    ...
    while filepieces:
        ...
        try:
            ...
            return (
                h5file,
                h5file["/".join([""] + list(objpieces)) if objpieces else "/"],
            )
    else:
...
def get_iteration_entry(h5object, n_iter):
    ...
    if n_iter < obj_iter_start or n_iter >= obj_iter_stop:
        raise IndexError(
            "data for iteration {} not available in dataset {!r}".format(
                n_iter, h5object
            )
        )
...
class WESTPAH5File(h5py.File):
    ...
    def __init__(self, *args, **kwargs):
        ...
        arg_fileformat_version = kwargs.pop(
            "westpa_fileformat_version", self._this_fileformat_version
        )
        ...
        h5file_iter_prec = _get_one_attr(
            self, ["westpa_iter_prec", "west_iter_prec", "wemd_iter_prec"], None
        )
        ...
        self.iter_prec = (
            h5file_iter_prec if h5file_iter_prec is not None else arg_iter_prec
        )
        self.fileformat_version = (
            h5file_fileformat_version
            if h5file_fileformat_version is not None
            else arg_fileformat_version
        )
    ...
    def iter_object_name(self, n_iter, prefix="", suffix=""):
        ...
        return "{prefix}iter_{n_iter:0{prec}d}{suffix}".format(
            n_iter=n_iter, prefix=prefix, suffix=suffix, prec=self.iter_prec
        )
...
class SingleSegmentDSSpec(SingleDSSpec):
    def get_iter_data(self, n_iter, seg_slice=index_exp[:]):
        if self.slice:
            return self.h5file.get_iter_group(n_iter)[self.dsname][
                seg_slice + index_exp[:] + self.slice
            ]
        else:
            return self.h5file.get_iter_group(n_iter)[self.dsname][seg_slice]
    ...
    def get_segment_data(self, n_iter, seg_id):
        if self.slice:
            return self.h5file.get_iter_group(n_iter)[
                numpy.index_exp[seg_id, :] + self.slice
            ]
        else:
    ...
    def get_iter_data(self, n_iter, seg_slice=index_exp[:]):
        ...
        for iset, dset in enumerate(datasets):
            if nsegs is None:
                ...
                raise TypeError(
                    "dataset {} has incorrect first dimension (number of segments)".format(
                        self.dsspecs[iset]
                    )
                )
            if npts is None:
                ...
                raise TypeError(
                    "dataset {} has incorrect second dimension (number of time points)".format(
                        self.dsspecs[iset]
                    )
                )
            ...
            if dset.ndim < 2:
                ...
                raise TypeError(
                    "dataset {} has too few dimensions".format(self.dsspecs[iset])
                )
            elif dset.ndim > 3:
                # array per timepoint
                raise TypeError(
                    "dataset {} has too many dimensions".format(self.dsspecs[iset])
                )
            elif dset.ndim == 2:
...
class IterBlockedDataset:
    ...
    def empty_like(cls, blocked_dataset):
        ...
        source = (
            blocked_dataset.data
            if blocked_dataset.data is not None
            else blocked_dataset.dataset
        )

        newbds = cls(
            numpy.empty(source.shape, source.dtype),
            attrs={
                "iter_start": blocked_dataset.iter_start,
    ...
    def __init__(self, dataset_or_array, attrs=None):
        try:
            ...
            if attrs is None:
                raise ValueError(
                    "attribute dictionary containing iteration bounds must be provided"
                )
    ...
    def iter_entry(self, n_iter):
        if n_iter < self.iter_start:
            raise IndexError(
                "requested iteration {} less than first stored iteration {}".format(
    ...
    def iter_slice(self, start=None, stop=None):
        ...
        if start < self.iter_start:
            raise IndexError(
                "requested start {} less than stored start {}".format(
