...
from westpa.stringmethod import WESTStringMethod, DefaultStringMethod
...
class StringDriver:
    def __init__(self, sim_manager, plugin_config):
        ...
        try:
            ...
            log.error(
                "String Driver Error: Failed during initialization of string method: {}".format(
                    e
                )
            )
            raise
        ...
        sim_manager.register_callback(
            sim_manager.prepare_new_iteration, self.prepare_new_iteration, self.priority
        )

        rc.pstatus("-westext.stringmethod -----------------\n")

        rc.pstatus("windowsize: {}\n".format(self.windowsize))

        rc.pstatus("update interval: {}\n".format(self.update_interval))

        rc.pstatus("initial update: {}\n".format(self.initial_update))

        rc.pstatus("priority: {}\n".format(self.priority))

        rc.pstatus("write average positions: {}\n".format(self.write_avg_pos))

        rc.pstatus("do update: {}\n".format(self.do_update))

        rc.pstatus("initialize from WE data: {}\n".format(self.init_from_data))
        ...
        with self.data_manager.lock:
            ...
            if self.init_from_data:
                ...
                try:
                    ...
                    log.warning(
                        "Initializing string centers from data failed; Using definition in system instead."
                    )
    ...
    def update_bin_mapper(self):
        ...
        rc.pstatus("westext.stringmethod: Updating bin mapper\n")
        rc.pflush()
        ...
        try:
            ...
            self.system.bin_mapper = VoronoiBinMapper(
                self.dfunc, self.strings.centers, dfargs=dfargs, dfkwargs=dfkwargs
            )
        except (ValueError, TypeError) as e:
            log.error(
                "StringDriver Error: Failed updating the bin mapper: {}".format(e)
            )
            raise
    ...
    def avgpos_cartesian(self, n_iter):
        ...
        for n in range(start_iter, stop_iter):
            with self.data_manager.lock:
                ...
                sum_bin_weight += np.bincount(
                    bin_indices.astype(np.int), weights=weights, minlength=nbins
                )
    ...
    def prepare_new_iteration(self):
        ...
        if (
            n_iter - last_update < self.update_interval
            or n_iter < self.initial_update
            or not self.do_update
        ):
        ...
        rc.pstatus(
            "westext.stringmethod: Calculating average position in string images\n"
        )
