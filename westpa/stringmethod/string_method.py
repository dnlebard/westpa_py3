import numpy as np
from westpa.stringmethod.fourier_fitting import FourierFit
...
from westpa.stringmethod import WESTStringMethod
    ...
    def __init__(
        ...
        **kwargs
        ...
        if slen and isinstance(slen, Iterable):
            ...
            log.warning(
                "Input parameters do not define slen; assuming system is composed of a single string"
            )
        ...
        start_count = 0
        for sl in slen:
            self._strindx.append(
                np.index_exp[start_count : (start_count + sl), self._indx_take]
            )
            start_count += sl
    ...
    def update_string_centers(self, avgcoords, binprob):
        ...
        **Parameters**
        ...
        for sid, si in enumerate(self._strindx):
                ...
                if self._SCIPY_FLAG:
                    for k in range(self._ndim_take):
                        f = scipy.interpolate.interp1d(
                            cfunc(~notocc), x[~notocc, k], kind="linear"
                        )
                        x[notocc, k] = f(cfunc(notocc))
                else:
                    for k in range(self._ndim_take):
                        x[notocc, k] = np.interp(
                            cfunc(notocc), cfunc(~notocc), x[~notocc, k]
                        )
            ...
            if self._SCIPY_FLAG:
                for k in range(self._ndim_take):
                    psi_new[:, k] = scipy.linalg.solve_banded(
                        (1, 1), self._A[N], b[:, k]
                    )
            else:
                for k in range(self._ndim_take):
