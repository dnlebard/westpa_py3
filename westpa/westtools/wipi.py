...
from westpa.westtools import Plotter
...
class KineticsIteration:
    def __init__(self, kin_h5file, index, assign, iteration=-1):
        ...
        for key in _1D_h5keys:
            try:
                ...
                self.__dict__[key] = None
        try:
            self.__dict__["total_fluxes"] = WIPIDataset(
                raw=np.array(self.h5file["total_fluxes"]), key="total_fluxes"
            )
    ...
    class __custom_dataset__:
        ...
        def _1D_repr_pretty_(self, p, cycle):
            ...
            # Stolen shamelessly from westpa.westtools/kinetics_tool.py
    ...
    def __2D_with_error__(self, h5key, index, assign):
        ...
        self.step_iter = (
            self.h5file[h5key]["iter_stop"][0] - self.h5file[h5key]["iter_start"][0]
        )[1, 0]
...
class __get_data_for_iteration__:
    ...
    def __init__(self, parent, value, seg_ids=None):
        ...
        current["seg_id"] = np.array(list(range(0, iter_group["seg_index"].shape[0])))[
            seg_ids
        ]
        current["walkers"] = current["summary"]["n_particles"]
        current["states"] = parent.assign["trajlabels"][
            value - 1, : current["walkers"], :
        ][seg_ids]
        ...
        current["direct"] = KineticsIteration(
            parent.direct, value, parent.assign, value
        )
        evolution_datasets = [
            "rate_evolution",
        ...
        try:
            current["reweight"] = KineticsIteration(
                parent.reweight, value, parent.assign, value
            )
            ...
            current["instant_matrix"] = sp.coo_matrix(
                (matrix["flux"][...], (matrix["rows"][...], matrix["cols"][...])),
                shape=((nbins - 1) * 2, (nbins - 1) * 2),
            ).todense()
            reweighting = True
        except:
    ...
    def successful_trajectories(self):
        ...
        state_changes = np.where(
            self.raw["states"][:, :-1] != self.raw["states"][:, 1:]
        )
    ...
    def __getitem__(self, value):
        ...
        if isinstance(value, str):
            ...
            if value < self.walkers:
                ...
                print(
                    "INVALID SEG_ID {}.  SEG_ID should be less than {}.".format(
                        value, self.walkers
                    )
                )
...
class WIPIScheme:
    ...
    def past(self):
        ...
        """
        if self.iteration > 1:
            return __get_data_for_iteration__(
                value=self.iteration - 1, seg_ids=self.current["parents"], parent=self
            )
        else:
