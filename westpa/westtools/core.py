...
class WESTTool(WESTToolComponent):
    ...
    def make_parser(
        self, prog=None, usage=None, description=None, epilog=None, args=None
    ):
...
class _WESTSubcommandHelp(WESTSubcommand):
    ...
    def add_args(self, parser):
        parser.add_argument(
            "command",
            nargs="?",
            choices=[subcommand.subcommand for subcommand in self.parent.subcommands],
        )
    ...
    def go(self):
        if self.command is None:
            ...
            self.parent._subcommand_instances[self.command].subparser.print_help(
                sys.stdout
            )
        sys.exit(0)
...
class WESTMasterCommand(WESTTool):
    ...
    def __init__(self):
        ...
        self._subcommand_instances = {
            subcommand_class.subcommand: subcommand_class(self)
        ...
        if __debug__:
            for scclass in self.subcommands:
                assert (
                    scclass.subcommand
                ), 'subcommand {!r} does not define a "subcommand" class variable'.format(
                    scclass
                )
