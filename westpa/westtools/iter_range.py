...
from westpa.westtools.core import WESTToolComponent
import numpy
...
class IterRangeSelection(WESTToolComponent):
    ...
    def __init__(self, data_manager=None):
        ...
        self.include_args.update(
            {"iter_start": True, "iter_stop": True, "iter_step": False}
        )
    ...
    def process_args(
        self,
        args,
        override_iter_start=None,
        override_iter_stop=None,
        default_iter_step=1,
    ):
        ...
        if override_iter_stop is not None:
            ...
            self.iter_stop = (
                self.data_manager or westpa.rc.get_data_manager()
            ).current_iteration
    ...
    def slice_per_iter_data(
        self, dataset, iter_start=None, iter_stop=None, iter_step=None, axis=0
    ):
        ...
        if (
            iter_start < ds_iter_start
            or iter_stop > ds_iter_stop
            or ds_iter_step % iter_step > 0
        ):
            raise IndexError(
                (
                    "Cannot slice requested iterations [{:d},{:d}) (stride={:d}) from dataset {!r}"
                    + "with range [{:d},{:d}) (stride={:d}).".format(
                        iter_start,
                        iter_stop,
                        iter_step,
                        ds_iter_start,
        ...
        for idim in range(len(dataset.shape)):
            if idim == axis:
                dimslices.append(
                    slice(
                        iter_start - ds_iter_start,
                        iter_stop - ds_iter_stop + iter_step,
                        iter_step,
                    )
                )
            else:
    ...
    def iter_range(self, iter_start=None, iter_stop=None, iter_step=None, dtype=None):
        ...
        return numpy.arange(
            ...
            iter_stop,
            iter_step,
            dtype=(dtype or numpy.min_scalar_type(iter_stop)),
        )
