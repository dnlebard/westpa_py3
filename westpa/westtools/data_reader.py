...
class WESTDataReader(WESTToolComponent):
    ...
    the main HDF5 file from westpa.cfg or command line arguments, caching of certain
    ...
    def weight_dsspec(self):
        if self._weight_dsspec is None:
            ...
            self._weight_dsspec = SingleIterDSSpec(
                self.we_h5filename, "seg_index", slice=index_exp["weight"]
            )
...
class WESTDSSynthesizer(WESTToolComponent):
    ...
    def add_args(self, parser):
        igroup = parser.add_argument_group(
            self.group_name
        ).add_mutually_exclusive_group(required=not bool(self.default_dsname))
    ...
    def process_args(self, args):
        if args.construct_dataset:
            self.dsspec = FnDSSpec(
                self.h5filename, get_object(args.construct_dataset, path=["."])
            )
        elif args.dsspecs:
