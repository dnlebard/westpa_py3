...
class Plotter:
    ...
    def __generic_ci__(
        ...
        if (interface == None and self.interface == "text") or interface == "text":
            ...
            try:
                ...
                if self.dim == 3:
                    plt.plot(
                        h5file[h5key]["expected"][:iteration, i, j] / tau, color="black"
                    )
                    plt.plot(
                        h5file[h5key]["ci_ubound"][:iteration, i, j] / tau, color="grey"
                    )
                    plt.plot(
                        h5file[h5key]["ci_lbound"][:iteration, i, j] / tau, color="grey"
                    )
                else:
                    plt.plot(
                        h5file[h5key]["expected"][:iteration, i] / tau, color="black"
                    )
                    plt.plot(
                        h5file[h5key]["ci_ubound"][:iteration, i] / tau, color="grey"
                    )
                    plt.plot(
                        h5file[h5key]["ci_lbound"][:iteration, i] / tau, color="grey"
                    )
                plt.show()
            except:
                print(
                    "Unable to import plotting interface.  An X server ($DISPLAY) is required."
                )
                if self.dim > 1:
    ...
    def __generic_histo__(self, vector, labels):
        ...
        if self.interface == "text":
            ...
            try:
                ...
                plt.bar(
                    list(range(0, np.array(vector).shape[0])),
                    vector,
                    linewidth=0,
                    align="center",
                    color="gold",
                ...
                for x in range(0, cols):
                    if x == 0:
                        with self.t.location(0, y):
                            print(
                                self.t.red("{0:.4f}|".format(float(h - y) / float(h)))
                            )
    ...
    def __terminal_ci__(self, h5file, iteration, si, sj, tau, h5key):
        ...
        scale = np.array(
            [0.0] + [ylower + i * (yupper - ylower) / np.float(h) for i in range(0, h)]
        )[::-1]
        if iteration > w:
                ...
                for x in range(0, w - 12):
                    ...
                    if x == 0:
                        for y in range(0, h + 1):
                            with self.t.location(0, y):
                                print(
                                    self.t.bold(self.t.red("{0:.7f}|".format(scale[y])))
                                )
                    ...
                    print(
                        self.t.move(
                            np.digitize(h5file[h5key]["expected"][in_tup] / tau, scale),
                            x + 12,
                        )
                        + self.t.on_blue("-")
                    )
                ...
                if self.dim == 3:
                    print(
                        "{} from {} to {} from iter 1 to {}".format(
                            h5key,
                            self.state_labels[si],
                            self.state_labels[sj],
                            self.iteration,
                        )
                    )
                else:
                    print(
                        "{} of state {} from iter 1 to {}".format(
                            h5key, self.state_labels[si], self.iteration
                        )
                    )
    ...
    def __terminal_expected__(self, h5file, iteration, si, sj, tau, h5key, dim):
        ...
        if yupper > 0:
            ...
            scale = np.array(
                [0.0]
                + [ylower + i * (yupper - ylower) / np.float(h) for i in range(0, h)]
            )[::-1]
        else:
            yupper = (np.max(h5file) / tau) / 2
            ...
            try:
                for x in range(0, w - 12):
                    ...
                    for y in range(ci[0], ci[1]):
                        ...
                        # print(self.t.on_blue(' '))
                    print(
                        self.t.move(np.digitize(h5file[in_tup] / tau, scale), x + 12)
                        + self.t.on_blue("-")
                    )
