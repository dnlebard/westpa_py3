...
def probAdjustEquil(
    binProb, rates, uncert, threshold=0.0, fullCalcClust=False, fullCalcBins=False
):
    ...
    if fullCalcBins:
        ...
        ratios_indirect = UncertMath.UncertContainer(
            ri_vals, ri_min, ri_max, mask=ratios_direct.vals.mask
        )
    ...
    ias = ma.argsort(
        uncertij, fill_value=np.inf
    )  # Get the indices that would sort uncertij
