...
def prob_adjust(binprob, rates, uncert, oldindex, targets=[]):
    ...
    new_zi = np.where(mapped_new_weights == 0.0)[
        0
    ]  # bins that after reweight have zero prob
    ...
    new_nzi = np.nonzero(mapped_new_weights)[
        0
    ]  # bins that after reweight have non-zero prob
