...
class BinningMixin(AnalysisMixin):
    ...
    def process_args(self, args, upcall=True):
        if args.binexpr:
            westpa.rc.pstatus(
                "Constructing rectilinear bin boundaries from the following expression: '{}'".format(
                    args.binexpr
                )
            )
    ...
    def write_bin_labels(
        ...
        for (ibin, label) in enumerate(self.mapper.labels):
            dest.write(
                format.format(bin_index=ibin, label=label, max_iwidth=max_iwidth)
            )
    ...
    def assign_to_bins(self):
        ...
        assignments = numpy.zeros(
            (n_iters, max_n_segs, pcoord_len), numpy.min_scalar_type(self.n_bins)
        )
        ...
        for (iiter, n_iter) in enumerate(range(self.first_iter, self.last_iter + 1)):
            ...
            for seg_id in range(len(seg_index)):
                assignments[iiter, seg_id, :] = self.mapper.assign(
                    pcoords[seg_id, :, :]
                )

            for it in range(pcoord_len):
                populations[iiter, it, :] = numpy.bincount(
                    assignments[iiter, : len(seg_index), it],
                    weights,
        ...
        assignments_ds = self.binning_h5group.create_dataset(
            "bin_assignments", data=assignments, compression="gzip"
        )
        populations_ds = self.binning_h5group.create_dataset(
            "bin_populations", data=populations, compression="gzip"
        )
    ...
    def require_bin_assignments(self):
        ...
        if self.discard_bin_assignments:
            ...
            westpa.rc.pstatus(
                "Existing bin assignments are for incompatible first/last iterations; deleting assignments."
            )
            do_assign = True
        elif not self.check_data_binhash(self.binning_h5group):
            westpa.rc.pstatus(
                "Bin definitions have changed; deleting existing bin assignments."
            )
            do_assign = True
    ...
    def get_bin_assignments(self, first_iter=None, last_iter=None):
        return self.slice_per_iter_data(
            self.binning_h5group["bin_assignments"], first_iter, last_iter
        )

    def get_bin_populations(self, first_iter=None, last_iter=None):
        return self.slice_per_iter_data(
            self.binning_h5group["bin_populations"], first_iter, last_iter
        )
