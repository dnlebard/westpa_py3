...
class KineticsAnalysisMixin(AnalysisMixin):
    ...
    def process_args(self, args, upcall=True):
        ...
        if args.ibins_string:
            ...
            westpa.rc.pstatus(
                "Will calculate kinetics data from transitions beginning in any bin."
            )
        ...
        if args.fbins_string:
            ...
            westpa.rc.pstatus(
                "Will calculate kinetics data from transitions ending in any bin."
            )
    ...
    def selected_bin_pair_iter(self):
        return (
            ...
            for pair in itertools.product(
                self.analysis_initial_bins, self.analysis_final_bins
            )
        )
