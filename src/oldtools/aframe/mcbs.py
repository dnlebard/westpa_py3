...
class MCBSMixin(AnalysisMixin):
    ...
    def process_args(self, args, upcall=True):
        ...
        self.mcbs_nsets = (
            args.mcbs_size
            if args.mcbs_nsets
            else min(1000, calc_mcbs_nsets(self.mcbs_alpha))
        )
        self.mcbs_display_confidence = "{:.{cp}f}".format(
            100 * args.mcbs_confidence,
...
def calc_ci_bound_indices(n_sets, alpha):
    return (
        int(math.floor(n_sets * alpha / 2)),
        int(math.ceil(n_sets * (1 - alpha / 2))),
    )


def bootstrap_ci_ll(
    estimator, data, alpha, n_sets, storage, sort, eargs=(), ekwargs={}, fhat=None
):
...
def bootstrap_ci(
    estimator, data, alpha, n_sets=None, sort=numpy.msort, eargs=(), ekwargs={}
):
    ...
    try:
        return bootstrap_ci_ll(
            estimator,
            data,
            alpha,
            n_sets or calc_mcbs_nsets(alpha),
            fsynth,
            sort,
            eargs,
            ekwargs,
