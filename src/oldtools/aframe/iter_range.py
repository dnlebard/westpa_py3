...
class IterRangeMixin(AnalysisMixin):
    ...
    def check_iter_range(self):
        ...
        if (
            self.last_iter is None
            or self.last_iter > self.data_manager.current_iteration - 1
        ):
    ...
    def slice_per_iter_data(
        self, dataset, first_iter=None, last_iter=None, iter_step=None, axis=0
    ):
        ...
        if (
            first_iter < ds_first_iter
            or last_iter > ds_last_iter
        ...
        for idim in range(len(dataset.shape)):
            if idim == axis:
                dimslices.append(
                    slice(
                        first_iter - ds_first_iter,
                        last_iter - ds_first_iter + iter_step,
                        iter_step,
                    )
                )
            else:
