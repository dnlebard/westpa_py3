...
class TrajWalker:
    ...
    def trace_to_root(self, n_iter, seg_id):
        ...
        while segment.p_parent_id >= 0:
            segment = self.data_reader.get_segments_by_id(
                segment.n_iter - 1, [segment.p_parent_id]
            )[0]
            segments.append(segment)
    ...
    def get_trajectory_roots(self, first_iter, last_iter, include_pcoords=True):
        ...
        for n_iter in range(first_iter, last_iter + 1):
            ...
            segments = self.data_reader.get_segments_by_id(
                n_iter, seg_ids, include_pcoords=include_pcoords
            )
    ...
    def trace_trajectories(
        ...
        if (get_state or set_state) and not (get_state and set_state):
            raise ValueError(
                "either both or neither of get_state/set_state must be specified"
            )
