...
class WESTDataReaderMixin(AnalysisMixin):
    ...
    def process_args(self, args, upcall=True):
        if args.west_h5name:
            ...
            self.west_h5name = westpa.rc.config.get_path(
                ["west", "data", "west_data_file"]
            )
    ...
    def clear_run_cache(self):
        ...
        del (
            self.__c_iter_groups,
            self.__c_seg_id_ranges,
            self.__c_seg_indices,
            self.__c_parent_arrays,
            self.__c_parent_arrays,
        )
    ...
    def get_segments(self, n_iter, include_pcoords=True):
        ...
        return self.get_segments_by_id(
            n_iter, self.get_seg_ids(n_iter, None), include_pcoords
        )
    ...
    def get_seg_index(self, n_iter):
        try:
            ...
            seg_index = self.__c_seg_indices[n_iter] = self.get_iter_group(n_iter)[
                "seg_index"
            ][...]
            return seg_index
    ...
    def get_wtg_parent_array(self, n_iter):
        try:
            ...
            parent_array = self.__c_wtg_parent_arrays[n_iter] = self.get_iter_group(
                n_iter
            )["wtgraph"][...]
            return parent_array
    ...
    def get_pcoord_array(self, n_iter):
        try:
            ...
            pcoords = self.__c_pcoord_arrays[n_iter] = self.get_iter_group(n_iter)[
                "pcoord"
            ][...]
            return pcoords
    ...
    def get_pcoord_dataset(self, n_iter):
        try:
            ...
            pcoord_ds = self.__c_pcoord_datasets[n_iter] = self.get_iter_group(n_iter)[
                "pcoord"
            ]
            return pcoord_ds
    ...
    def get_seg_ids(self, n_iter, bool_array=None):
        try:
            ...
            all_ids = self.__c_seg_id_ranges[n_iter] = numpy.arange(
                0, len(self.get_seg_index(n_iter)), dtype=numpy.uint32
            )
...
class ExtDataReaderMixin(AnalysisMixin):
    ...
    def process_args(self, args, upcall=True):
        ...
        if args.usecols:
            westpa.rc.pstatus(
                "Using only the following columns from external input: {!s}".format(
                    args.usecols
                )
            )
    ...
    def text_to_h5dataset(
        ...
        for line in fileobj:
            ...
            if not ncols_input:
                ...
                raise ValueError(
                    "expected {:d} columns at line {:d}, but found {:d}".format(
                        ncols_input, linenumber, len(fields)
                    )
                )
            ...
            if not ncols_store:
                ...
                dataset = group.create_dataset(
                    dsname,
                    shape=(0, ncols_store),
                    maxshape=(None, ncols_store),
                    chunks=(chunksize, ncols_store),
                    dtype=dtype,
                )
    ...
    def npy_to_h5dataset(self, array, group, dsname, usecols=None, chunksize=None):
        ...
        if usecols:
            for istart in range(0, maxlen, chunksize):
                ...
                westpa.rc.pstatus(
                    "\r  Read {:{mw}d}/{:>{mw}d} rows".format(iend, maxlen, mw=mw),
                    end="",
                )
                westpa.rc.pflush()
        else:
...
class BFDataManager(AnalysisMixin):
    ...
    traj_index_dtype = numpy.dtype(
        [("pcoord_len", numpy.uint64), ("source_data", h5py.special_dtype(vlen=str))]
    )
    ...
    def require_bf_h5file(self):
        if self.bf_h5file is None:
            ...
            try:
                ...
                self.bf_h5file.create_dataset(
                    "traj_index",
                    shape=(0,),
                    maxshape=(None,),
                    dtype=self.traj_index_dtype,
                )
