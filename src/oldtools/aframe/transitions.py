...
class TransitionEventAccumulator:
    ...
    def __init__(self, n_bins, output_group, calc_fpts=True):
        ...
        self.timepoint = (
            None  # current time index for separate calls on same trajectory
        )
        self.last_bin = (
            None  # last region occupied, for separate calls on same trajectory
        )
        self.last_bin_pop = (
            None  # total weight in self.last_region at end of last processing step
        )
    ...
    def record_transition_data(self, tdat):
        ...
        if len(tdat) + self.tdat_buffer_offset > self.tdat_buffersize:
            ...
            self.tdat_buffer[
                self.tdat_buffer_offset : (self.tdat_buffer_offset + len(tdat))
            ] = tdat
    ...
    def start_accumulation(self, assignments, weights, bin_pops, traj=0, n_iter=0):
        ...
        self._accumulate_transitions(
            timepoints, assignments, weights, bin_pops, traj, n_iter
        )
    ...
    def continue_accumulation(self, assignments, weights, bin_pops, traj=0, n_iter=0):
        ...
        self._accumulate_transitions(
            timepoints, aug_assign, aug_weights, aug_pops, traj, n_iter
        )

    def _accumulate_transitions(
        self, timepoints, assignments, weights, bin_pops, traj, n_iter
    ):
        tdat = []
        ...
        for (trans_ti, weight, ibin, fbin, ibinpops) in zip(
            ...
            durations = (
                -last_exit + trans_ti + 1
            )  # = time now - time of exit from initial bin
            if calc_fpts:
                fpts = (
                    -last_completion[fbin, :] + trans_ti
                )  # = time now - time of last final->initial transition
...
class TransitionAnalysisMixin(AnalysisMixin):
    ...
    def require_transitions(self):
        ...
        if self.discard_transition_data:
            ...
            westpa.rc.pstatus(
                "Bin definitions have changed; deleting existing transition data."
            )
            do_trans = True
        elif (
            "transitions" in self.trans_h5group
            and not self.check_data_iter_range_least(self.trans_h5group)
        ):
            westpa.rc.pstatus(
                "Existing transition data is for different first/last iterations; deleting."
            )
            do_trans = True
    ...
    def find_transitions(self):
        ...
        self.accumulator = TransitionEventAccumulator(
            self.n_bins, output_group, calc_fpts=self.calc_fpts
        )
        ...
        for h5object in (
            output_group,
            output_group["n_trans"],
            output_group["transitions"],
        ):
    ...
    def _segment_callback(self, segment, children, history):
        ...
        if len(history) == 0:
            ...
            self.accumulator.start_accumulation(
                self.bin_assignments[iiter, seg_id, :],
                weights,
                bin_pops,
                traj=self.n_trajs,
        ...
        if not self.__quiet_mode and (
            self.n_segs_visited % 1000 == 0 or self.n_segs_visited == self.n_total_segs
        ):
            pct_visited = self.n_segs_visited / self.n_total_segs * 100
            westpa.rc.pstatus(
                "\r  {:d} of {:d} segments ({:.1f}%) analyzed ({:d} independent trajectories)".format(
                    int(self.n_segs_visited),
                    int(self.n_total_segs),
                    float(pct_visited),
                    self.n_trajs,
                ),
                end="",
...
class BFTransitionAnalysisMixin(TransitionAnalysisMixin):
    ...
    def find_transitions(self, chunksize=65536):
        ...
        self.accumulator = TransitionEventAccumulator(
            self.n_bins, output_group, calc_fpts=True
        )
        ...
        for traj_id in range(self.get_n_trajs()):
            ...
            for istart in range(0, nrows, chunksize):
                ...
                if istart == 0:
                    self.accumulator.start_accumulation(
                        assignments, weights, binpops, traj=traj_id
                    )
                else:
                    self.accumulator.continue_accumulation(
                        assignments, weights, binpops, traj=traj_id
